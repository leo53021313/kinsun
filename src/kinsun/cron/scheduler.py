...
class Job:
    ...
    **整輪掃描停住**。當晚 `daily-consolidation` 對 39 位長輩跑整理＋摘要＋反思時，
    ...
    """遲到多久就算「已經造成損害」；`None` ＝ 用後台的預設容許量。

    ⚠️ 為什麼不能全部共用一個容許量（2026-07-27）：後台 `/admin/jobs` 原本一律用
    300 秒判逾期，而 `schedule-dispatch` 每分鐘跑、判定窗只有 **90 秒**且明文
    「停機超過此窗即作廢，**不補**」。於是遲到 90～300 秒這段區間裡，
    **提醒已經永久遺失了，後台卻還顯示健康**——那正是這一頁該抓卻抓不到的地方。

    這個值是 job 自己的性質（它遲到多久會開始掉東西），故由建構它的工廠填，
    不由後台猜。日更型的清理／批次 job 留 `None` 即可：它們晚幾分鐘沒有損害，
    用嚴格的容許量只會製造假警報。
    """
