...
⚠️ 殘留限制一：容忍度只把自我實現迴圈的門檻從 30 分抬到 60 分，沒有根治它。若她固定
在問候後 90 分（> 容忍度）才回話，時間仍會逐日往後漂，最後由上限接住。要根治得比對
問候實際送出的時刻，而 first_user_turn_per_day 這個訊號沒有那個資訊——已知限制。

⚠️ 殘留限制二：死區只擋得住「問候迴圈」，擋不住**其他 job 誘發的發言**。死區比對的是
問候時間，而系統還有別的排程會在同一個時間帶對她說話——失聯關心（
PROACTIVE_INACTIVITY_HOUR 預設 10）正落在護軌 [6, 11] 的正中間，用藥（8／12）與
回診（8）也在帶內。

first_user_turn_per_day 只取 `role='user'`（金孫自己的問候不算「她醒了」），但她**回覆**
失聯關心產生的正是一則 `role='user'` turn——在資料上與「她自己醒來開口」**完全無法
區分**，而死區看不到這條路徑。

要命的是這兩群人高度重疊：**會忽略問候的長輩，恰恰就是會收到失聯關心的那群**。於是
她每天唯一的發言是 10:05 回覆我們 10:00 的關心 → 中位數 ≈ 10:05 → 問候從 08:00 逐晚
往後，三晚後收斂在 09:30（已實測）。那個 09:30 純粹由**我們自己的排程**決定，她真正
的起床時間從未被量到。

護軌把結果關在 [6, 11] 內、不傷人（不會半夜三點問候），故接受。要根治同樣需要「系統
在幾點對她說過話」這個資訊——與限制一是同一個根因：訊號裡沒有我們自己的足跡。
"""
