...
# 通報排在回覆生成之前（pipeline.py），卡住的代價是長輩連回覆都拿不到。本專案為同型的
# 「呼叫沒設逾時、對端沒回應就無限期等待」付過學費：llm.py 的 Gemini 逾時存進欄位卻從沒
# 傳給 SDK；db.py 也曾因連線沒設 TCP keepalive 而讓死連線無限期等待（⚠️ 但那不是
# 2026-07-26 排程器靜默停擺七小時的根因——那次根因是 try_claim 浮點等值比對，與逾時
# 無關，見 docs/dev/14_部署與運維.md v1.17）。
#
