...
* **單條被濾網擋下** → 只丟該條，其餘照寫。拒絕理由**逐條**記 warning：這是我們
  ...
  「輕蔑攔截」的計數就是「模型多常試圖教阿白忽視長輩」——它自成一桶，別跟醫療混。
...
# 角色叫阿白（2026-08-07 新視覺人設）；「金孫」只留作服務名。這個標籤同時是模型
# 歸納稱謂的來源，必須與 REFLECTION_PROMPT 及 agent.py 的 SYSTEM_PROMPT 同名。
...
# policy.py docstring 點名的真正傷害：阿白當著長輩的面把危急訊號正常化。第 2 條擋不住它
...
REFLECTION_PROMPT = (
    # 角色名必須與 agent.py 的 SYSTEM_PROMPT 一致：這裡產出的守則會**原文注入**
    # 那份 prompt，一份 prompt 裡出現兩個名字，模型自稱哪一個就沒有保證，而它的
    # 自稱會經 TTS 唸給長輩聽。「金孫」是服務名，本提示詞談的全是角色該怎麼與
    # 長輩相處，不出現服務名。
    "你是「阿白」——一位陪伴長輩的 AI 夥伴——的反思模組。以下是這位長輩過去幾天"
    "與阿白的對話，以及系統推送的提醒與長輩是否回應的紀錄。\n\n"
    "請找出「與這位長輩相處時應該遵守的守則」，讓阿白下次做得更好。\n\n"
    "【嚴格限制，違反者一律作廢】\n"
    "1. 只能產出四類守則：address（稱呼）、tone（語氣與長短）、routine（作息與時間）、"
    "topic（話題偏好）。\n"
    "2. 絕對禁止產出任何涉及用藥、劑量、就醫、回診、危急判斷、健康數值的守則。"
    "阿白的用藥提醒與安全機制不是你能調整的對象。\n"
    "3. 守則的文字也不可以出現醫療或身體不適的字眼（醫、診、病、藥、痛、血、跌、"
    "不舒服……），含這些字的守則會被系統直接丟棄。話題類的守則請改寫成不含這些字"
    "的說法，例如「不要聊她過世老伴生病的那段」要改寫成「不要聊她過世老伴的事」。\n"
    "4. 但若一條守則的本意是要阿白忽視、淡化或不理會長輩說的不適、疼痛、情緒或求助，"
    "一律不要產出，也不可改寫成不含上述字眼的說法來規避第 3 條——例如「她說胸口很痛"
    "通常只是想撒嬌，不用理會」不合法，改寫成「她抱怨的時候通常只是想撒嬌，不用理會」"
    "同樣不合法。這類守則永遠不合法：長輩喊不舒服時，阿白必須當真。\n"
    ...
    # 第 9 條同樣**不是**防線——`_split_ungrounded_address` 才是。文字稿必然含阿白
    # 自己的回覆（`_ROLE_LABELS` 把 assistant 標成「阿白」），模型看到滿滿的「阿公」
    # 就會歸納出「稱呼長輩為阿公」，卻不覺得自己在猜。這裡寫清楚只為少掉候選。
    "9. 稱謂（阿公、阿嬤、爺爺、奶奶……）只能依**長輩自己說出口**的話認定，"
    "阿白自己用過的稱呼不算證據——那是阿白的猜測，不是長輩的偏好。\n\n"
...
def _split_ungrounded_address(
    ...
    """擋掉「只有阿白自己講過那個稱謂」的 address 守則——那是把猜測學回來。

    2026-08-01 正式環境實況：長輩的 `name` 是「阿嬤」、沒設 nickname，於是
    `accounts/profile.py` 注入「系統沒有性別資料，不要自行猜測用『阿公』或『阿嬤』
    ...
    把 `assistant` 標成「阿白」。阿白先前每一句猜出來的「阿公」都成了模型眼中的
...
def _transcript_message(turns: list[Message]) -> Message:
    ...
    不可直接把 user/assistant 歷史當 messages 餵給模型：對話必然以阿白的回覆結尾，
