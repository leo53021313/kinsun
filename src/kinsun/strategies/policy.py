...
這道交集正是誤殺控制的本體：「她講古時會重複同一件事，聽聽就好」（失智照護的標準做法）
沒有訴求訊號，放行；「她心情不好時聽聽就好」有，攔下。
...
* **「不用緊張」**：會誤殺「她講話慢，不用緊張催她」（講的是別催她，不是別理她）。只收
  帶淡化語氣的「不用太緊張」，且需搭配訴求訊號。
...
# 訴求訊號：長輩把「我不好」講出來的樣子。單獨出現完全合法（「她難過的時候多陪她講
# 幾句」），它只是下面那張表的觸發條件。
_COMPLAINT_SIGNALS = (
    "抱怨",
    "訴苦",
