...
* 環境變數：
  ...
  * 鍵一律掛子系統前綴：`GEMINI_`、`LONGTERM_`、`PROACTIVE_`、`REFLECTION_`（每晚反思與策略記憶）、`SCHEDULER_`、`ASR_`、`TTS_`、`AUDIO_`、`SUPABASE_`（專案層憑證）、`LIFF_`、`LINE_`、`INTERNAL_TESTING_`（內測模式旗標）、`NEWS_`（話題新聞，D-74）、`TDX_`（交通 TDX 憑證）、`OPIK_`（工程觀測）、`TURN_`（對講機容量）。
