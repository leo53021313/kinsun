...
* 環境變數：
  ...
  * 鍵一律掛子系統前綴：`GEMINI_`、`LONGTERM_`、`PROACTIVE_`、`REFLECTION_`（每晚反思與策略記憶）、`SCHEDULER_`、`ASR_`、`TTS_`、`AUDIO_`、`SUPABASE_`（專案層憑證）、`LIFF_`、`LINE_`、`INTERNAL_TESTING_`（內測模式旗標）、`NEWS_`（話題新聞，D-74）、`TDX_`（交通 TDX 憑證）、`OPIK_`（工程觀測）、`TURN_`（對講機容量與單輪預算）。
...
**指不出「出錯的那一行」，就還沒找到根因。** 合格的根因說明必須同時具備：

1. 在**目前的程式碼**上重現症狀
2. 指出**確切那一行**，並說明它為什麼產生這個症狀
3. 說明你的修改**如何改變那一行的行為**

做不到第 2 點就不要動手，也不要在文件或 commit 訊息裡寫下根因。
**寫「推測」遠好過寫一個看起來很有把握的錯誤根因**——後者會讓下一個人沿著錯的方向找。

三種常見的錯誤前提：

* **「這個限制像是沒做完」** — 它常常是刻意的，先用 `git log -p -S "<符號>"` 讀當初的意圖。
* **「這個空缺該補起來」** — 缺席可能是承重的，補上去反而破壞它正在保護的事。
* **「我的心智模型是對的」** — 最貴的一種。追程式碼與執行期實況，不要靠印象。

驗證時的兩條紀律：

* **拿到證據先問「它還能支持什麼別的解釋」**，再決定它支不支持你的結論。
  一個現象往往同時符合好幾種假設，挑最先想到的那個寫進文件，代價由後面的人付。
* **驗證「往返」要走完真正的來回**，含資料庫、序列化、網路每一段。
  **只涵蓋一半的實測比沒有實測更危險**——它會給你「已經驗證過」的錯覺。

根因涉及外部系統（資料庫、外部 API、作業系統）時，必須在**那個系統上**驗證。
「單元測試全綠、正式環境全掛」是最常見的組合。
...
`docs/dev/` 為系統現況與契約的唯一開發文檔，**必須隨程式碼保持最新**：

* 任何程式碼改動完成時，必須**同一時間**檢查 `docs/dev/` 是否受影響，受影響的文件立即更新，不可累積事後補。
* 「改動 → 該更新哪些文件」對照表見 `docs/dev/15_文檔與維護指南.md` §2 同步規則。
* 有更新任一文件時，一併更新該文件版頭（版本／日期）與 `docs/dev/README.md` 文件狀態表。
