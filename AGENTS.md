...
驗證時的兩條紀律（2026-07-26 排程器停擺 13.8 天的教訓）：

* **證據先問「還能支持什麼別的解釋」**，再決定它支不支持你的結論。當時看到連線
  `Recv-Q` 有積壓就推論「對端消失」，但那個現象的語意其實是「資料到了沒人讀」，
  方向相反——錯誤的根因因此寫進了文件，也修錯了地方。
* **驗證「往返」要走完真正的來回**，含資料庫、序列化、網路每一段。當時實測過
  「epoch 往返沒有精度損失」，但只測了 Python 端；真正失效的是 SQL 端的 `WHERE`
  比對，那一半從頭到尾沒測。**只涵蓋一半的實測比沒有實測更危險**，它給你已經
  驗證過的錯覺。

根因涉及外部系統（DB、外部 API、作業系統）時，必須在**那個系統上**驗證——
這次的單元測試綠燈了 13 天，而正式環境全停。
