 ...
 *
 * ⚠️ 網頁端搬移新增：`web/tsconfig.json` 同時載入 `DOM`（`lib`）與 `node`（`types`），
 * 全域 `setTimeout` 因而是兩份宣告合併的多載函式，單獨 `ReturnType<typeof setTimeout>`
 * 只會取到其中一種（`Timeout`），而測試裡的假計時器慣例回傳陣列索引（`number`）——這裡
 * 補上 `number` 純粹是型別加寬以涵蓋兩種環境的不透明代號，不影響任何執行期行為。
 */
