...
export const strings = {
  ...
  guardianHome: {
    ...
    // 在畫面上，「左邊」在窄螢幕（組員拿手機自測）是錯的方向指示——對面那句
    // `elderBind.receivedFromGuardian` 已為同一件事拿掉「右邊」。
    sendToElder: "送到長輩的手機",
  ...
  voiceProfile: {
    ...
    // ⚠️ 明講「整個唸完」而不只說「太短」：家屬看到「太短」的直覺是隨便補幾個字，
    ...
    revoke: "改回預設聲音",
    revokeConfirm: "長輩下一次講話就會聽到原本的聲音。確定改回預設嗎？",
    revoked: "已經改回預設聲音了。",
    ...
    // ⚠️ 六種麥克風失敗要分開講：一律顯示「請到設定開啟權限」的話，沒有麥克風的
    // 桌機、用非 https 網址連進來的人，都會去找一個根本不存在的開關（同長輩端
    // MicrophoneProbeResult 由兩種擴充成六種的理由）。
  ...
  elderBind: {
    ...
    // 後端不認這支 token（401）時，長輩會被送回這個畫面，這句話告訴他發生什麼事、
    // 下一步做什麼。⚠️ 最常見的成因就是家人剛按了「重新產生長輩綁定碼」——他手上
    // 那組新碼這時正拿在手裡（見 elderDetail.bindingWarning 對家屬的保證），所以
    // 直接叫長輩去掃／去打那一組，不要只講「登入失效」這種他無從下手的話。
    // 另一個成因是彩排後重建資料庫，長輩看到的指示同樣成立。
  ...
  elderLogin: {
    ...
    phoneLabel: "手機號碼",
    // 返回鍵講「回去哪裡」而不是「返回」——長輩端每一句都要告訴他下一步。
    // 從這個畫面按返回是回到配對畫面（輸入家人給的號碼），故不是「回去講話」。
    back: "回去輸入號碼",
  },
  talk: {
    ...
    // 實際行為相反的文案比沒有文案更糟。補播時字幕跟著真的播出來的那一則走，
    // 長輩看得到這句話在回答什麼，不需要另外一句說明。
  ...
  elderNotifications: {
    ...
    loadFailed: "現在讀不到您的提醒，這不代表今天沒有事。請等一下再看一次，或打電話問家人。",
    back: "回去講話",
