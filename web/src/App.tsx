 ...
 *
 * ⚠️ **運營狀態的 hook 住在這裡而不是 GatePage 裡面**：TearTransition 在 active
 * 翻轉時會把 children 從原位換到 overlay 底下的兩份副本，那會卸載活著的 GatePage、
 * 掛上兩個全新實例。狀態留在 GatePage 裡的話，撕開的那一瞬間畫面會閃回
 * 「正在確認服務狀態…」、按鈕變灰，並且對後端多打兩次——而那正是整個展示的
 * 門面動畫，看的人會以為它壞了。
 */
...
function Demo() {
  ...
  // ⚠️ 已經站在舞台上就代表撕裂播完了，把旗標收回來。以前它住在 `<Routes>` 底下的
  ...
  //   訂閱者），`setTearing(false)` 會單獨先繪製一次，那一格 `tearing` 與 `onStage`
  ...
  return (
    <>
      {tearing || onStage ? <StagePage /> : null}
      {onStage ? null : (
        <TearTransition active={tearing} onDone={enterStage}>
          <GatePage state={gate} onStart={() => setTearing(true)} />
        </TearTransition>
      )}
