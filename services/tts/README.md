...
## 參考語音的錄製準則（2026-08-09 實測；2026-08-19 修訂）

同時適用於**全域金孫聲音**（`TTS_PROMPT_WAV`）與**長輩客製化聲音**（家屬錄的那段）。
第 1～4 點皆為在 DGX 8012 實機 A/B 測出來的，不是推測；第 5 點為尚未解決的已知限制。

> ⚠️ **2026-08-19 修訂（優先於下方第 1、3、4 點）**：長逐字稿會把**稿子內容滲進回覆**。
> 實測（同一長句合成 4 次、ASR 回聽）：40 字逐字稿滲漏 2/4～3/4——兩份不同語速的
> 錄音皆滲、滲出的字永遠是稿子的字（家屬回報的症狀＝回覆「一直講阿嬤」）；
> 16 字稿與官方 15 字樣本（`zero_shot_prompt.wav`）皆 0/4。第 4 點當時記錄
> 「聽不出品質差異，只是變慢」的結論**已被推翻**：差異是機率性的，單次聽不到。
> 故客製化聲音的固定稿（`voice_profiles/script.py`）已改為 16 字、且**不含
> 阿嬤／阿公等稱謂**（滲漏出來就是叫錯人）——代價是放棄第 1 點的「嬤」發音示範，
> 若阿嬤稱謂發音明顯退化再另行處理。第 3 點的「10～15 秒較穩」與本修訂相衝，
> 取捨採官方樣本的量級（約 5 秒）：官方樣本作為全域預設聲音在正式環境數月未見
> 長度不穩，滲漏則是實聽可辨的事故。
...
- [x] 長輩客製化聲音複製（2026-07-30）：`/synthesize` 支援選填 `elder_id`／`prompt_audio_url`／
      ...
      表與 `VoicePipeline` 串接已實作。
- [x] **家屬自行設定的入口**（2026-08-11）：`GET /voice-profile-script` 下發照唸稿、
      `PUT`／`GET`／`DELETE /elders/{id}/voice-profile`（見 `docs/dev/06_API設計規範.md`）。
      在此之前只有機制沒有入口，唯一的建立方式是直接寫資料庫——「每位長輩可有專屬聲音」
      實際等於「全系統一個聲音」。
      **端到端實測通過**（2026-08-11，DGX 8012）：同一位長輩連續三輪，服務日誌顯示
      設定前用全域參考逐字稿、設定後改用系統朗讀稿（＝該長輩的設定）、撤銷後回全域，
      且 `/tmp/voice-<elder_id>.wav` 確實產生（下載＋ffmpeg 正規化皆執行）。
      三段音檔人耳確認：預設是 A 的聲音、設定後換成 B 的聲音、撤銷後回 A。
- [ ] **（交付 Leo）** 錄製並定調正式**金孫參考語音**——作為缺客製化設定檔時長輩聽到的
      ...
      技術面的準備已完成，錄音者照著做即可（準則詳見上方「參考語音的錄製準則」，
      **以 2026-08-19 修訂為準**）：
      - 講話語調有起伏的人，約 5～7 秒
      - 逐字稿 16 字上下、**不含阿嬤／阿公等稱謂**（長稿會滲進回覆、稱謂滲漏＝叫錯人）
      - **「喔」出現兩次（一次句中、一次句尾），且最後一字後留白一秒再停止錄音**
        ——這是為了同時驗證上方已知限制第 5 條的推測
      - 建議稿即客製化聲音的固定稿（`voice_profiles/script.py`）：
        「你好呀，今天天氣真好喔，記得多喝水喔。」
