...
- 圖片型海報、掃描 PDF、影片逐字稿不列入 ingestion；本階段明確不做 OCR。
...
本報告是 2026-06-29 的驗證快照，表格內容不回頭改寫；治理狀態的唯一真實來源是
`src/kinsun/rag/source_registry.py`。表與程式不一致時以程式為準，差異記於此節。

| 日期 | 變更 | 理由 |
|---|---|---|
| 2026-08-02 | `cdc_advocacy`（CDC 宣導，`needs_review`／`conditional`）更名為 `cdc_diseases`，改指 `https://www.cdc.gov.tw/Disease`，治理狀態改為 `allowed`／`approved` | 本報告當初擔心的「圖片、PDF、影片與授權聲明」是宣導專區的特性；2026-08-01 實測該專區只有一個索引頁，爬回來的 17 份「文件」全是站台選單、零篇文章。改指的傳染病介紹頁是純 HTML 內文（實測 10 頁皆有 473～2,288 字），著作權聲明與 `cdc_home` 同一份，故沿用其 `allowed` 判定。內容經由 CDC 官方 RSS（`type=2`）取得 97 個疾病頁。 |
| 2026-08-02 | 新增 `cdc_vaccination`（疾管署預防接種專區，`allowed`／`approved`） | 疫苗接種指引與傳染病介紹是兩塊內容。`cdc_diseases` 抓回來的是疾病本身的說明，「疫苗」只出現在附件標題；全庫同時含「接種」與「長者」的 chunk 僅 5 個且全部不相干。預防接種專區有〈成人預防接種建議時程表〉〈成人肺炎鏈球菌疫苗接種 Q&A〉〈流感疫苗〉等頁（實測剝除後各 2,785／3,663／3,121 字）。著作權與 `cdc_home`／`cdc_diseases` 同一份聲明。該區沒有 RSS 也沒有 sitemap，走連結爬取。 |
| 2026-08-02 | 新增 `hpa_sleep_handbook`（國健署〈睡眠與精神健康〉手冊 PDF，`allowed`／`approved`），新增網域 `health.hpa.gov.tw` | 長者睡眠衛教在既有來源全數落空：國健署 sitemap 的 5,667 篇裡標題含「睡」「眠」的 14 篇全是嬰兒猝死與寶寶睡姿；衛福部有兩篇對的（睡眠呼吸中止症、夜夜好眠）但埋在焦點新聞第 1,094 頁分頁（約 21,880 篇）裡，任何合理的爬取預算都到不了。本手冊為文字型 PDF（非掃描檔），實測抽出 18,968 字，符合本報告「只允許文字型 PDF」的既有原則。⚠️ 單一檔案來源、30 MB，每次週更會重抓。 |
| 2026-08-02 | 評估後未新增 `mohw_focus_news`（衛福部焦點新聞） | 一度為了上述兩篇睡眠文章而新增，實測後撤除：焦點新聞有 1,094 頁分頁，目標文章在深處，`RAG_CRAWLER_MAX_PAGES=100` 永遠到不了。留著只會多收一百篇近期新聞稿。 |
| 2026-08-02 | ⚠️ 待處理：`mohw_health_window`／`mohw_health_article`／`mohw_health_list` 三個來源實質無產出 | 實測三者收回來的全是「109年衛生福利部新聞」這類年度索引頁、零篇內文；`mohw_health_window` 的種子頁 `np-34-1.html` 標題雖是「衛教視窗」，整頁沒有任何文章連結。三者又同爬一個網域、跨來源去重先到先得，後兩者實際貢獻為零。與 HPA 四來源合併（2026-08-01）是同一種病，尚未處理。→ 已於 2026-08-05 處理，見下列。 |
| 2026-08-05 | 三個 mohw 來源合併為 `mohw_health_education`（Leo 核定），舊三者改 `out_of_scope`／`approved_for_rag=False` 保留供稽核 | 新來源指向衛生福利 e 寶箱（`lp-88-1-40.html`），以 `content_url_pattern=cp-88-\d+` 收其底下 30 個主題頁（用藥安全、健康促進、醫療照護、傳染病防治、中醫藥⋯⋯），實測各 1,100～1,800 字、子連結只有頁尾，主題頁本身即是內容。治法與 HPA 四來源合併相同。 |
