...
Run: `npm --prefix web run build && grep -rn "unpkg\|jsdelivr\|cdn\." web/dist/assets/*.js | head`
Expected: 沒有任何輸出（若有，代表 wasm 位址沒有覆寫成功，回到 Step 1 重看套件 API）
...
export function BindScreen(props: {
  ...
  async function submit(raw: string) {
    ...
    try {
      ...
      signIn({ role: "elder", token: session.token, display_name: session.name });
      props.onDone();
    } catch (exc) {
      setError(
