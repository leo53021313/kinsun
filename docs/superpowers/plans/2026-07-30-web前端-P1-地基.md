...
⚠️ **`include` 刻意不含 `vite.config.ts`**，與既有的 `frontend/tsconfig.json` 一致。設定檔裡的 `node:path` 與 `__dirname` 需要 `@types/node` 才通得過型別檢查，而姊妹專案已經用「不把設定檔納入檢查」解決過同一個問題——為此新增一個依賴不符合專案規範「除非有充分理由否則不新增第三方套件」的門檻。代價是那 25 行打錯字要等到 `npm run dev` 或 `npm run build` 當下才發現（Vite 載入設定時會直接炸，訊息很明確），與 `frontend/` 目前承擔的風險相同。
...
const client = createApiClient({
  // ⚠️ 用展開的 if 而非三元運算式：三元的兩個分支被 TypeScript 合成聯集後，空物件
  // 那支會變成 `{ Authorization?: undefined }`，而 `undefined` 塞不進
  // `Record<string, string>` 的索引簽章（TS2322）。`app/src/lib/api.ts` 與
  // `frontend/src/api.ts` 的同一個位置都是展開寫的——這是本 repo 對同一個型別限制
  // 的既定寫法。
  authHeaders: (token) => {
...
type SessionContextValue = {
  ...
  /** ⚠️ 不收 `role`：角色由工廠決定，呼叫端傳不進來。收 `Session` 的話，
   *  在長輩 context 上以 `role: "guardian"` 呼叫會把 token 存進家屬的鍵——
   *  畫面顯示長輩已登入、重整後卻變成家屬登入，那是「兩欄互不干擾」唯一可被
   *  違反的路徑（P1 全分支審查實測確認）。 */
...
export function createSessionContext(role: Role) {
  ...
  function Provider(props: { children: ReactNode }) {
    ...
    const signIn = useCallback((next: Omit<Session, "role">) => {
      // ⚠️ `role` 放在展開之後：寫成 `{ role, ...next }` 的話，呼叫端若繞過型別
      // 硬塞一個 role 就會蓋掉工廠的角色，而型別檢查抓不到。
