...
## KinSun Prototype Decisions

- Visual source of truth: `references/source-visual.png`.
- Product surface: native-style elder companion conversation screen.
- Preserve the selected direction's warm companionship, very large Traditional Chinese type, and one dominant 104dp press-and-hold voice action.
- Blend in the approved reference language through ink-charcoal outlines, sun yellow, sky blue, coral orange, rounded controls, and a flat geometric A-Kin character.
- The prototype covers Idle, Listening, Thinking, Speaking, network-error recovery, and a non-destructive logout demonstration.
- Mirror the production talk gesture contract: a short tap starts listening and the next tap sends; holding for at least 500ms sends on release.
- Every visible action or link affordance must be operable in the prototype; do not leave dead controls.
- This folder is an isolated design prototype. Do not connect it to the production API, authentication, microphone, persistence, or the production Expo application.
