...
> **版本:** v1.16 | **更新:** 2026-07-30 | **狀態:** ✅ 定稿（App 12 頁＋LIFF 3 頁＋admin 7 頁；D-76 統一 `/schedules` 路由；`/elder/notifications` 長輩提醒頁；對講機 WS 五態與純文字降級收尾契約、A-Kin 視覺）
...
graph TB
    subgraph AppEnd["App（Expo，session 存 SecureStore＋SessionProvider）"]
        ...
        subgraph GuardianLine["家屬線（家屬 token）"]
            ...
            H --> HS["/guardian/elder/:id/schedules 行程管理（用藥／回診／其他，✅ D-76）"]
            G --> N["/guardian/notifications 通知列表（✅ D-12）"]
        end
    end

    subgraph LiffEnd["LIFF /liff（idToken，凍結維運）"]
        LE["/ 長輩清單"] --> LS["/elders/:id/schedules 行程 CRUD"] & LH["健康報告"]
    end
    ...
    subgraph API["後端 /api/v1（信封 success/data/error/meta）"]
        ...
        TURN["對講機：WS /ws/talk（ack／reply／error）＋POST /turns 降級"]
        REST["照護資源：elders｜schedules｜health-report｜daily-summaries｜notifications｜elder-notifications｜account"]
...
**頁面總計**：App **12 頁**＋LIFF **3 頁**（清單／統一行程／健康報告）＋admin **7 頁**（總覽／訊息流／長輩／長輩詳情／鏈路詳情／話題新聞／系統）。（沿革：2026-07-12 App 曾以用藥、回診兩個子頁計 12 頁；D-76 將兩頁合併為 `/schedules` 後減 1，2026-07-29 X-01 新增 `/elder/notifications` 再加 1，故現況仍為 12。LIFF 同理由 4→3；admin 於 D-74 新增話題新聞頁後為 7。）
...
`/guardian/login` → `/guardian/home 長輩列表` → `/guardian/elder/:id`（健康報告＋每日摘要＋全部行程＋邀請碼）。to-be：詳情頁掛「新事件」提示（D-12 App 內通知落點）；✅ D-09 已定：家屬可見＝警報＋提醒＋**每日摘要**，逐字對話不開放。
...
| 項目 | 內容 |
| :--- | :--- |
| 職責 | 說話→自動播回覆；以正式 A-Kin 插畫、彩色狀態帶與文字顯示金孫狀態（idle/listening/thinking/speaking/error） |
| 主要 CTA | 圓形麥克風鍵（104dp、只放既有 `MicIcon` 向量圖示、鍵內無文字）＋鍵下方大型動作提示；支援**兩種說話手勢**（2026-07-25）：①按住說話（達 `delayLongPress` 500ms 門檻，放開送出）②短按切換（按一下開始聆聽、說完再按一下送出，聆聽中顯示「說完再按一下」提示）。手勢由純邏輯狀態機 `lib/talkGesture.ts` 判定（含單元測試）——不可用 avatar state 判斷手勢：短按時 pressOut 比重繪先到會讀到過期值，曾造成「聆聽中殘留＋二次按壓 `prepareToRecordAsync` 洗掉音檔→識別失敗」bug；停止前先 await 開錄 promise 消除競態 |
| 回覆顯示 | 整頁採 **ScrollView**：短回覆、A-Kin、狀態帶與 CTA 在 393×852 邏輯尺寸內完整可見；長回覆或大系統字級仍可上滑看完整，字級維持 elder 最小 22pt token 不縮小。 |
...
| WS 狀態收尾 | 收到 `ack` 播安撫語音時顯示 speaking，播完回 **thinking** 等正式答案；`reply` 最後一段播完才回 idle；仍有分段時維持 speaking；`error` 一律進 error，不可偽裝成「準備好了」。若 TTS 降級為 `audio_url=""` 的純文字 reply，訊框抵達即回 idle，不等待不存在的播放完成事件。狀態決策集中於 `getTalkSocketPlaybackCompletionState`／`getTalkSocketFrameArrivalState` 並有單元測試。 |
...
| 錯誤狀態 | 一律 replyText 人話；狀態帶切成高對比錯誤色並顯示重新說明；403 綁定失效→清 session＋引導重綁 |
| 內測 | `internalTesting` 為真時，頂部顯示麥克風／定位授權狀態列（正式環境不顯示） |
| to-be | A-Kin 靜態插畫已在不改公開 `AvatarPlaceholder` 契約下落地；未來若導入 Rive／Live2D，只替換元件內部與對嘴層（D-08 預留）。錄音觸覺＋音效回饋已落地（✅ D-48 丁-2）。 |
...
| 項目 | 內容 |
| :--- | :--- |
| 職責 | 健康報告（近 30 天危急＋提醒）＋**每日摘要**（✅ D-09 己-3 已落地）＋全部行程（用藥／回診／其他清單與管理入口，✅ D-76）＋家屬邀請碼＋**長輩帳密代辦**（✅ D-71 己-6） |
| 資料需求 | `GET /health-report`、`GET /daily-summaries`、`GET /elders/:id/schedules`、`POST /guardian-invites`、`PUT /account`；`useFocusEffect` 返回重載（管理頁編輯後即反映） |
| to-be | 詳情頁「新事件」badge（D-12 通知頁已有，詳情頁未做） |

### /guardian/elder/:id/schedules 行程管理（✅ D-76 統一排程）

| 項目 | 內容 |
| :--- | :--- |
| 職責 | 用藥／回診／其他提醒的新增、編輯、刪除；同頁顯示全部 group，並標出「長輩自己交代的」項目；刪除掛系統確認框 |
| 入口 | 詳情頁「管理行程」按鈕 |
| 資料需求 | `/elders/:id/schedules` 的 GET／POST，以及 `/elders/:id/schedules/:group_id` 的 PUT／DELETE（家屬 token） |
| 輸入 | 類型單選（吃藥／回診／其他）＋提醒內容；用藥可選四時段或直接輸入 `HH:MM`，回診輸入 `YYYY-MM-DD HH:MM`（時間可省），其他輸入每天／每週／單次規則 |
| 錯誤狀態 | 前端檢查內容與時間格式；API 錯誤以信封繁中訊息顯示，401 走共用登出流程 |
...
| `/guardian/elder/:id/schedules` | App | 家屬 token | 家屬（✅ D-76 統一排程：用藥／回診／其他） |
...
- [x] 頁面清單與程式一致（2026-07-30 實查：App **12 頁**、LIFF **3 頁**、admin **7 頁**；D-76 統一 `/schedules`，X-01 補 `/elder/notifications`）
...
| v1.13 | 2026-07-29 | App 12→13 頁：新增 `/elder/notifications` 長輩提醒頁，並在對講機加入鈴鐺與未讀 badge（X-01）。 |
| v1.14 | 2026-07-30 | `/elder/talk` 正式整合核准 Prototype：A-Kin 靜態插畫、五態彩色狀態帶、Phosphor 狀態／登出圖示與鍵下動作提示落地；同步保留既有麥克風／鈴鐺向量圖示、WS 長連線、`POST /turns` 降級、提醒鈴鐺與位置契約；整頁可捲動並以 Android 393×852 驗證核心操作完整可見。 |
| v1.15 | 2026-07-30 | 依程式路由重算為 App 12／LIFF 3／admin 7；D-76 的 `/schedules` 取代用藥與回診分頁。補 WS 視覺狀態收尾契約：ack 播完維持 thinking、reply 最後一段才 idle、error 進高對比 error。 |
| v1.16 | 2026-07-30 | 補 WS 純文字 reply 收尾：後端 TTS 降級回 `audio_url=""` 時，畫面直接離開 thinking；有語音仍等播放完成。 |
