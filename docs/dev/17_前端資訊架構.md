...
三端（App／LIFF／admin）的頁面、旅程、導航與 URL 的單一真實來源；`web/`（2026-07-30 新增，App／LIFF 凍結後唯一演進的前端）的路由另見 §7。家屬框內六畫面已於 P2（2026-07-31）落地並補入本文件（§3／§6）；長輩框內四畫面中，配對與帳密重登兩個已於 P3 Task 7（2026-07-31）落地並補入本文件，對講機與提醒兩個仍待 Task 8／9 落地。
...
graph TB
    ...
    subgraph WebEnd["web `/demo/stage`（雙欄手機外框，session 存 localStorage＋雙工廠）"]
        ...
        subgraph WebElder["長輩欄（左欄，✅ 配對／登入 P3 Task 7 完成；對講機／提醒 ⏳ 待 Task 8／9）"]
            WEB["elder/bind 配對（掃 QR 或輸入號碼）"] -.->|用過金孫？帳密登入| WEL["elder/login 帳密重登（✅ D-71 己-6）"]
            WEL --> WET
            WEB --> WET["elder/talk 對講機（⏳ 佔位，待 Task 8）"]
...
### web/ 長輩端畫面（`/demo/stage` 左欄，配對／帳密重登 ✅ P3 Task 7 完成，2026-07-31）
...
| 對講機 | `elder/talk` | ⏳ 待 Task 8 | — | 目前為佔位分支（`strings.common.comingSoon`），配對／登入成功後會先看到這句話 |
| 提醒 | `elder/notifications` | ⏳ 待 Task 9 | — | 同上，佔位分支 |
