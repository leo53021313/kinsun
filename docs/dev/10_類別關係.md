...
classDiagram
    ...
    class RiskDetector { -classifier: RiskClassifier -mid=0.4 +assess(text) +combine_with_llm(text, llm) }
    ...
    class CombinedSafetyClassifier { <<Protocol>> +classify(text) CombinedSafetyResult }
    class LlmCombinedSafetyClassifier { -llm: LLMClient +classify(text) 一次呼叫取兩份原始判斷 }
