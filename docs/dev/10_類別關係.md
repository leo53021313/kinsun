...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.31 | 2026-08-07 | **記憶檢索延遲優化 Task 1-3 複審修正**：§3 FactProvider 表列的 `ScheduleFacts×3（用藥／回診／自訂，D-76）` 更正為 `ScheduleFacts`（一次查詢供三段，D-76／記憶檢索延遲優化工項①），本清單自此為四路（碰 DB 三路，先前記六路／碰 DB 五路是三實例併一前的舊值）；§3／§5 兩處 `class FactProvider` mermaid 簽章 `+facts(elder_id) FactSection?` 放寬為 `FactSection\|list~FactSection~\|None`，對齊 `memory/recall.py` 現行型別。純文件修正，無 Schema 或行為變動 |
