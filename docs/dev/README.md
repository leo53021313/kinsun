...
| 07 | [模組規格與測試](07_模組規格與測試.md) | ✅ v1.61（Task 7 審查修正：`guardian/` 列補通知列表「錯誤取代空狀態」不變量、測試數 4→8（web 全套 163→167）；**新增 §7 前端模組（web/）**：網頁版前端 P2 家屬端完成，`guardian/`／`ui/`／`nav/`／`notify/` 首次併入本文件；C1 回覆音檔直送＋H-1 評測接線；延遲優化四刀＋審查修正；重數 Settings 102 欄／safety 11 檔／channels 11 檔／Protocol 50／後端測試 2511，六處 DbC 行號錨點更正） | 2026-07-31 |
