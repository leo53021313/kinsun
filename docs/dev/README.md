...
| 07 | [模組規格與測試](07_模組規格與測試.md) | ✅ v1.33（第二批：Gemini 逾時修復、mem0 檢索並行、CareAgent.prepare 情境預取，總測試 1747，端到端效益待配額重置後補量；第一批：新增 background.py、事實提供者並行查；RAG crawler 實戰修五瑕疵：WebForms、JSON 嗅探、垃圾連結、http 升級、URL 編碼＋NUL；統一排程五刀全完成、舊模組退役） | 2026-07-26 |
