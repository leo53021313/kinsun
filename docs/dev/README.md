...
| 07 | [模組規格與測試](07_模組規格與測試.md) | ✅ v1.60（**新增 §7 前端模組（web/）**：網頁版前端 P2 家屬端完成，`guardian/`／`ui/`／`nav/`／`notify/` 首次併入本文件；C1 回覆音檔直送＋H-1 評測接線；延遲優化四刀＋審查修正；重數 Settings 102 欄／safety 11 檔／channels 11 檔／Protocol 50／後端測試 2511，六處 DbC 行號錨點更正） | 2026-07-31 |
...
| 17 | [前端資訊架構](17_前端資訊架構.md) | ✅ v1.16（**網頁版前端 P2 家屬端完成**：§3 全景圖新增 web/ 家屬欄六畫面框內導覽子圖；§6 新增 web 家屬端六畫面規格；§7 附註更新（家屬框內畫面已補齊、長輩端仍待 P3）；§7 補 `/demo/stage` 伺服器端回退、舞台與狀態 hook 掛在路由之上兩則附註；**新增 /elder/notifications 長輩提醒頁＋對講機鈴鐺未讀 badge**（X-01）；對講機資料需求對齊 WS 長連線＋位置鍵名兩路徑一律 `location`；雙手勢＋admin 7 頁 D-74 話題新聞頁） | 2026-07-31 |
