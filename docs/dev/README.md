...
| 08 | [專案結構指南](08_專案結構指南.md) | ✅ v1.24（**網頁版前端 P2 家屬端完成回填**：§6 `web/src/` 樹補 `guardian/`（十檔）／`ui/`／`nav/`／`notify/`，移除過期的「P2／P3 補齊」承諾；網頁版前端 P1 地基：頂層新增 web/、§6 補 web/src 目錄樹；channels/app/inbound_audio.py＋safety/combined_classifier.py 入目錄樹；safety／channels 兩行補齊漏列檔案並附檔數） | 2026-07-31 |
