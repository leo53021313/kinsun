...
| 12 | [前端架構規範](12_前端架構規範.md) | ✅ v1.10（`signIn` 收 `Omit<Session, "role">`，角色由工廠決定；網頁版前端 P1 地基：三端擴為四端，新增 web/ 雙 session 工廠（與 App 單例的差異）與 CSP 兩處放寬理由；App 端接內嵌音檔：新增 lib/replyAudio.ts 落地 cache 暫存 .m4a、talkSocket 以注入點保住離線可測性；新增 expo-file-system 依賴，組員需重跑 npm install） | 2026-07-31 |
...
| 17 | [前端資訊架構](17_前端資訊架構.md) | ✅ v1.15（§7 補 `/demo/stage` 伺服器端回退、舞台與狀態 hook 掛在路由之上兩則附註；網頁版前端 P1 地基：§7 路由表新增 web/ 的 `/demo`／`/demo/stage` 兩條路由，框內長輩／家屬畫面待 P2／P3；**新增 /elder/notifications 長輩提醒頁＋對講機鈴鐺未讀 badge**（X-01）；對講機資料需求對齊 WS 長連線＋位置鍵名兩路徑一律 `location`；雙手勢＋admin 7 頁 D-74 話題新聞頁） | 2026-07-31 |
