...
> **版本:** v1.36 | **更新:** 2026-08-16 | **狀態:** ✅ 定稿（回覆新增 emotion 欄位，D-82）
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.31 | 2026-08-10 | 整合 PUT /elders/{elder_id}/persona 與 App transcript／WebSocket 呈現契約；本輪未新增 API，只收斂兩分支的現行契約。 |
| v1.30 | 2026-08-07 | App 對講成功回應新增 `transcript`：REST、WS JSON 與 WS binary header 一致回傳 ASR 原話，只供長輩本人裝置顯示「您說」與本機今日紀錄；家屬端 API 不新增逐字內容。同步更正 App 已移除過期 `getTurnChunk`／REST 續拉。 |
