...
| 兩顆**都已過**（今天的回診、且已過提醒鐘點） | `400` `invalid_schedule`＋`message`「這個回診的提醒時間都已經過了，請確認回診日期。」 |
...
**`meta.warnings`**（形狀比照 `GET /admin/jobs`）：`{"warnings": ["回診前一天的提醒時間（08:00）已經過了，這次只設定了回診當天 08:00 的提醒；前一天那次請您自己跟長輩提一聲。"]}`。沒有話要說時 `meta` 維持 `null`。**這是加法**——不讀它的既有 client 行為不變，但少建一顆鬧鐘的事實也就不會被家屬看見（⚠️ 目前四端皆未消費，見 12 §9 F-16）。
