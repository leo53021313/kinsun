...
| —（新增） | `WS /api/v1/ws/talk` | **對講機長連線**（spec 2026-07-28）：整輪對話走同一條 WebSocket，後端可主動送第二則訊息。認證走 query `token`（WebSocket 握手在 RN 與瀏覽器都不能自訂標頭），並以閘門複核同意，失敗以 close code 1008 關閉。上行：一個 binary 訊息＝一輪完整音檔；一個 JSON 訊息＝更新下一輪要用的位置（`{location, latitude, longitude}`，三者齊備才寫入）。下行皆帶 `turn_id`：`ack`（`text`／`audio_url`／`duration_ms`，模型決定要查東西時立刻送、音檔取自預錄語庫不現場合成）、`reply`（同 `POST /turns` 的欄位）、`error`（回退話術，含併發輪達上限時的婉拒）。同時在跑的輪數上限 3。⚠️ `POST /turns` 保留為降級路徑，兩者共存 |
