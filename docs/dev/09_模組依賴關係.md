...
| 原則 | 本專案落實 |
| :--- | :--- |
| 依賴倒置（DIP） | 49 個 Protocol＋三件套（Pg／Fake，21 組合約測試，新增 `VoiceProfileStore`）；Service 依賴介面不依賴實作 |
...
| v1.13 | 2026-07-30 | 長輩客製化聲音複製：§2 領域層補 `voice_profiles`（僅依賴 `db`）；§1 Protocol 48→49／合約 20→21 組（新增 `VoiceProfileStore`）；`VoicePipeline` 選配依賴 `VoiceProfileStore`（未設＝全域預設聲音），依賴方向向下、不新增循環。未經團隊拍板不入 00_決策清單。（合併 211 筆上游時本更新一度被覆蓋、此次重補） |
