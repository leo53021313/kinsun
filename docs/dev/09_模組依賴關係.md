...
- `channels/app/admission.py`（P3 Task 1，2026-07-31）比照上一筆 `inbound_audio.py` 的形狀：**同樣一個 kinsun 模組都沒有 import**（只用 stdlib 的 `itertools`／`threading`／`collections`／`contextlib`）。`TurnAdmission` 是純記憶體內的容量閘門，不依賴任何領域或基礎層模組；與 `inbound_audio.py` 不同的是它連執行期的鴨子型別依賴都沒有——本版**尚未被任何模組 import**（純新增，`turns.py`／`ws.py` 的接線與環境變數屬 Task 2），此刻與全庫其他模組之間完全沒有依賴邊，等 Task 2 接線後才會產生第一條邊。
