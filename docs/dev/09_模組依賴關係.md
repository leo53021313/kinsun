...
> **版本:** v1.9 | **更新:** 2026-07-30 | **狀態:** ✅ 定稿（隨程式現況滾動更新）
...
| 原則 | 本專案落實 |
| :--- | :--- |
| 依賴倒置（DIP） | 48 個 Protocol＋三件套（Pg／Fake，19 組合約測試，2026-07-30 增 VoiceProfileStore）；Service 依賴介面不依賴實作 |
...
graph TD
    ...
    subgraph 領域層
        ACC[accounts] & SCHED[schedules] & SAFE[safety] & MEM[memory] & RAG[rag] & REP[reports] & LOC[locations] & STRAT[strategies] & NEWS[news] & VOICE[voice_profiles]
    end
    subgraph 基礎層
...
| v1.9 | 2026-07-30 | 長輩客製化聲音複製：§2 領域層補 `voice_profiles`；§1 Protocol 47→48（新增 `VoiceProfileStore`），合約測試實數 19 組；未經拍板不入 00_決策清單。 |
