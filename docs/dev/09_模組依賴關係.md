...
graph TD
    ...
    組裝根 --> 邊緣層 --> 編排層 --> 領域層 --> 基礎層
    SAFE -. "⚠ notifier→router（D-18 反轉）" .-> ROUTER
```
...
`channels/app/turns.py`（邊緣）→ `channels/inbound.dispatch`（編排）→ `binding/gate.ConsentGate`（領域）→ `pipeline.VoicePipeline`（編排）→ `safety.RiskDetector`＋`agent.CareAgent`（領域）→ `memory.SessionMemory`（領域）→ `llm.GeminiClient`／`speech`／`db`（基礎）。全程依賴方向單向向下，僅 notifier 一處反向（D-18 修）。
...
| safety→channels 反向依賴 | ✅ D-18 Protocol 反轉（重構工項 A-1） |
