...
| 網頁版前端（web/） | 瀏覽器 runtime | React＋Vite（`web/src/`，由 API 進程託管靜態檔） | 現行（P1 地基＋P2 家屬端＋P3 對講機協定核心＋錄音播放與 iOS 音訊解鎖＋長輩端 QR 掃碼＋長輩端 API 呼叫端與文案＋長輩配對與帳密重登＋對講機＋長輩提醒列表（P3 收尾）＋**通知橫幅**（P4 Task 1＋審查修正，純新增尚未接線）＋**通知輪詢**（P4 Task 2＋審查修正，2026-07-31，純新增尚未接進畫面；審查修正：`bannerRef` 擋 `<StrictMode>` 雙呼叫、新增 `onTokenRevoked` 401 出口）＋**跨欄連動**（P4 Task 3，2026-07-31，`notify/bus.ts` 事件匯流排＋`prefilledCode` 發送端接線，`onTokenRevoked` 尚未接線待 Task 4）皆已完成；App／LIFF 凍結後唯一演進的前端，spec 2026-07-30） | 略：見 12／17 |
