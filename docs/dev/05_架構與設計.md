...
| voice_profiles | 長輩客製化聲音複製（聲音克隆，2026-07-30）：`VoiceProfileStore` 依 `elder_id` 存一組參考音檔＋逐字稿＋同意紀錄；`VoicePipeline._synthesize` 查出生效中設定檔即組 `VoiceReference` 傳給 TTS，無設定檔則沿用 DGX 端全域預設聲音。未經團隊拍板不入 00_決策清單 | `VoiceProfile`、`VoiceProfileStore`、`VoiceReference` |
