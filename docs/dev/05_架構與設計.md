...
> **版本:** v1.85 | **更新:** 2026-08-18 | **狀態:** ✅ 定稿（安撫話改為逐長輩，與客製化聲音一致；其餘沿用 v1.84）
...
⚠️ **安撫話用預錄語庫（`speech/acks.py`）而非模型生成**：原設計讓模型自己講，理由是話術貼合當下；55 筆語料推翻了這個前提——**65% 一字不差都是同一句「好，我幫您查一下喔」**（`web_search` 6/6、`search_nearby_places` 5/5 全部通用句），剩下 35% 的變化全部來自**工具**而非對話。而模型生成要多付一次 TTS（1.86 秒）並帶來兩種只有它才有的失效：把內部敘述寫進答案通道（實錄「碎碎唸：呼叫 \`get_news\` 工具…」，攔在 SDK HTTP 層 dump wire JSON 證實它與正常安撫話結構上無從區分、沒有任何旗標可辨識）、以及在工具還沒跑時就對長輩斷言結果。語庫為**人設 × 工具**兩維，兩軸各有保底；音檔不進版控、由當前 TTS 設定自動合成並快取（`speech/ack_audio.py`），換聲音只要重啟後端即自動重生。⚠️ **快取自 2026-08-18 起逐長輩分桶**：安撫話原本只有一份、以全域聲音合成，那在「全系統一個聲音」的年代與回覆一致；客製化聲音（`voice_profiles`）上線後前提失效——實機回報長輩在同一輪裡先聽到預設聲音的「我幫你查查」、再聽到家屬聲音的答案，像換了個人在講話。改為外層鍵＝長輩（`None`＝全域那批）、內層鍵＝句子；家屬設定聲音的當下由 API 觸發背景重暖（十九句約半分鐘，等到對話中才暖那位長輩前幾輪都不會有安撫話），撤銷時丟掉該桶。⚠️ 有專屬聲音但還沒暖好時**寧可不講也不用預設聲音頂替**——頂替就是那個要修掉的症狀本身；乾等是既有體感，換聲音是新缺陷，嚴重度不同。各桶效期互不連坐。涵蓋率由 `tests/test_speech_acks.py` 以 `pkgutil` 掃遍 `kinsun.tools` 強制把關——新增工具漏配 CI 就紅。
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.85 | 2026-08-18 | **安撫話改為逐長輩分桶**（實機回報：同一輪裡先聽到預設聲音的安撫話、再聽到家屬聲音的答案）：§ 非同步工具調用段補記快取結構、預熱時機（家屬設定的當下，非第一次對話）、「沒暖好寧可不講」的取捨與各桶效期不連坐。程式碼見 `speech/ack_audio.py`、`web/routers/voice_profiles.py` 的 `on_voice_changed`／`on_voice_revoked` 鉤子。 |
