...
> Worker 與 API **各自** `build_externals`（各一個 DB 連線池），共享同一 Supabase。VoicePipeline 只在 API 建——主動關懷走 `agent.proactive` 純文字（⚠ D-21 帶會議）。
...
- FR-4 危急偵測分級＋家屬通知（US-B1，政策 ⚠ D-10）；FR-5 App 內警報／提醒（US-B2，✅ D-12 to-be）
- FR-6 主動關懷（US-B3）；FR-7 用藥／回診提醒（US-C1／C2）
- FR-8 家屬帳號與綁定（US-D1／D2）；FR-9 健康報告（US-D3，範圍 ⚠ D-09）
...
| 分類 | 需求描述 | 目標值 |
| :--- | :--- | :--- |
| 性能 | 語音往返延遲 P50／P95 | ⚠ D-05 會議定（提案原 3–4 秒） |
...
| GuardianNotifier | 危急通知全家屬（`notifier.py:55`；⚠ 政策 D-10、依賴反轉 D-18） | accounts、router |
...
sequenceDiagram
    ...
    alt RiskTier ≥ L2
        ...
        loop 全部家屬（⚠ 齊發無冷卻＝D-10 會議）
            N->>R: send_text（L3 附 119 提示）
            R->>LINE: push
        end
    end
```
...
| 資料 | 分類 | 保留 | 依據 |
| :--- | :--- | :--- | :--- |
| 對話原文（turns）、危急事件、摘要 | 健康敏感 PII | 永久保留、同意文案明示 | ✅ D-14；撤回處置 ⚠ D-13 會議 |
