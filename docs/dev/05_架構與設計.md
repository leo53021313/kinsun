...
> API 服務同時託管 SPA 靜態檔（`_static_mounts`，`app.py:76-85／436-439`：`/liff`／`/admin`／`/demo` 三個掛載點，目錄不存在就不掛），故 SPA 下載來源＝API 進程；圖中省略該靜態檔箭頭。
