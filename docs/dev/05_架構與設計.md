...
> **版本:** v1.86 | **更新:** 2026-08-19 | **狀態:** ✅ 定稿（克隆逐字稿換 16 字短稿＋撤銷刪檔 prefixes 修正；其餘沿用 v1.85）
...
| voice_profiles | 長輩客製化聲音複製（聲音克隆，2026-07-30）：`VoiceProfileStore` 依 `elder_id` 存一組參考音檔＋逐字稿＋同意紀錄。**存的是 bucket 內的物件路徑（`prompt_audio_path`）而非簽章網址（2026-08-01）**：簽章網址預設 1 天就過期，過期後 DGX 端下載失敗會讓整輪退化成純文字、長輩完全沒聲音，且每輪重複（快取暖不起來）；改存路徑後由 `SupabaseAudioPublisher.signed_url_for` 每次現簽短效網址（效期 1 小時、行程內快取，實測首次約 384～620ms、快取命中 0ms）。**簽章快取以（路徑, 版本）為鍵（2026-08-19）**：版本＝`granted_at`、由 `resolve_voice` 傳入，版本換值即重簽新 token——`upload_voice_reference` 只清得掉收到 PUT 的那個 worker 的行程內快取，正式環境 `WEB_WORKERS` 預設 2，其他 worker 的舊網址（＋CDN 按完整 URL 的內容快取）不靠版本比對要活到效期結束，家屬重錄後長輩最長約 55 分鐘仍聽到舊聲音，且 DGX 端可能把舊音檔記在新版本底下、黏死到 TTS 服務重啟。簽章失敗或殘留舊網址值時記 ERROR 並退回全域預設聲音，不讓整輪陪葬。`VoicePipeline._synthesize` 查出生效中設定檔即組 `VoiceReference` 傳給 TTS，無設定檔則沿用 DGX 端全域預設聲音。**組裝於 `app.py` 建 `VoicePipeline` 時注入 `PgVoiceProfileStore`——未注入時 `resolve_voice` 一律回 None，設定檔存在也不會生效且全程無錯誤訊息**。分段串流的續段（`/turns/chunks/{index}`）不經 `speak`，自行呼叫 `VoicePipeline.resolve_voice(elder_id)` 取同一個聲音（2026-07-31 修：原本漏帶，第 0 段客製化、第 1 段起換回全域預設，長輩會聽到回覆講到一半換人）。未經團隊拍板不入 00_決策清單 | `VoiceProfile`、`VoiceProfileStore`、`VoiceReference` |
...
⚠️ **安撫話用預錄語庫（`speech/acks.py`）而非模型生成**：原設計讓模型自己講，理由是話術貼合當下；55 筆語料推翻了這個前提——**65% 一字不差都是同一句「好，我幫您查一下喔」**（`web_search` 6/6、`search_nearby_places` 5/5 全部通用句），剩下 35% 的變化全部來自**工具**而非對話。而模型生成要多付一次 TTS（1.86 秒）並帶來兩種只有它才有的失效：把內部敘述寫進答案通道（實錄「碎碎唸：呼叫 \`get_news\` 工具…」，攔在 SDK HTTP 層 dump wire JSON 證實它與正常安撫話結構上無從區分、沒有任何旗標可辨識）、以及在工具還沒跑時就對長輩斷言結果。語庫為**人設 × 工具**兩維，兩軸各有保底；音檔不進版控、由當前 TTS 設定自動合成並快取（`speech/ack_audio.py`），換聲音只要重啟後端即自動重生。**快取自 2026-08-19 起分聲音批次**（鍵＝`""` 全域預設／`elder_id@版本`）：有克隆聲音的長輩，過場語也用那個聲音——`ws.py::_run_turn` 開頭把 `resolve_voice` 丟背景執行緒（`_VoiceSlot` 交棒、announce 上限等 1 秒，安撫話回呼維持不查資料庫）並以 `ensure_warm` 在第一輪就預熱該長輩的批次；克隆批次未暖好時**寧可這輪不講也不退預設聲音**（同一輪過場預設聲、回答克隆聲＝講到一半換人），家屬重錄開暖新批次時同長輩舊版本批次整批丟棄；**家屬錄完（PUT）當下即由 voice_profiles router 觸發預熱**（同日追加，只暖得到收到 PUT 的 worker，另一 worker 靠開口時的 `ensure_warm` 補上）。回退話術（standby）維持預設聲音（管線失敗時投遞端拿不到聲音解析結果）。涵蓋率由 `tests/test_speech_acks.py` 以 `pkgutil` 掃遍 `kinsun.tools` 強制把關——新增工具漏配 CI 就紅。
...
**節流與安全**：`RateLimiter` Protocol＋雙實作（✅ 庚-08 A-54）：認證四個 scope（register／login／elder-login／bind，沿用 `AUTH_RATE_LIMIT_*`）正式組裝注入 `PgRateLimiter`（Postgres 共享滑動視窗，per-key `pg_advisory_xact_lock` 串行精確計數、跨進程、fail-open），記憶體版 `SlidingWindowRateLimiter` 對這四個 scope 只留測試／單 worker fallback——多 worker 上限不再 ×worker 數。⚠️ **2026-07-31 更正（P3 Task 2）**：此列先前只描述認證四個 scope，隱含「正式環境沒有任何記憶體版節流」——但第 **5 個 scope**（`turn:` 對講機每位長輩每分鐘輪數保險絲）正式組裝就是直接注入這個記憶體版（`app.py:405`，未升級為 Pg 版），多 worker 下上限**確實是** ×worker 數（與 §3.5／§3.6 資料庫連線池／對講機容量閘門同一種「設定值×worker 數」前提一致）；可接受是因為它對真人操作等同無限，走到那裡幾乎一定是前端在打自己（重連迴圈狂送），不是使用者在濫用。`security.py` 掛 5 個標頭（HSTS／nosniff／DENY／no-referrer／CSP）；⚠️ **2026-08-12 起有唯一一條逐字路徑例外**：`/demo/otto/renderer.html` 改掛 `SAMEORIGIN`＋獨立 CSP（`frame-ancestors 'self'`），否則網頁版的阿白會被自家標頭擋在 iframe 外、永遠停在 opacity-0。例外以 `scope["path"]` 逐字比對，不可改用 `request.url.path`（那會讓單頁應用殼也吃到放寬的標頭，見 13 §2）；admin `X-Admin-Key` 以 `hmac.compare_digest` 常數時間比對＋金鑰未設 fail-closed 503。
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.86 | 2026-08-19 | **克隆逐字稿滲漏修正＋撤銷刪檔修正**：固定稿由 40 字換 16 字且不含稱謂（長稿被 CosyVoice 機率性唸進回覆，實測 2/4～3/4；官方量級短稿 0/4）；Supabase 批次刪除鍵 `paths`→`prefixes`（撤銷刪檔上線以來靜默失敗、聲紋殘留，孤兒已補刪）。詳見 07 v2.19 與 services/tts/README.md 08-19 修訂。 |
| v1.85 | 2026-08-19 | **客製化聲音重錄失效修正＋安撫話分聲音批次**（Leo 需求：重錄後對話、續段與工具過場語都要立刻換新克隆聲音）：voice_profiles 列補簽章帶版本（多 worker／CDN 舊網址失效），安撫話段補 `AckAudioCache` 分聲音批次與 `ws.py` 背景預解析（`_VoiceSlot`／`ensure_warm`）。詳見 07 v2.18。 |
| v1.84 | 2026-08-12 | §節流與安全 補記 `security.py` 自 2026-08-12 起有唯一一條逐字路徑例外（`/demo/otto/renderer.html` 改掛 `SAMEORIGIN`＋獨立 CSP，否則網頁版阿白被自家標頭擋在 iframe 外）。此列先前仍寫「掛 5 個標頭（…DENY…）」、隱含全站一律不可嵌入，而 05 是架構的唯一真實來源——PR #109 改了 07／08／12／13 卻漏掉本檔，2026-08-12 審查時補上。一併載明例外必須以 `scope["path"]` 比對。 |
| v1.82 | 2026-08-10 | 整合 main 的金孫人設、地端 BGE-M3、觀測 span 補全與依賴修補，並保留 Jerry 的 Expo／web 新視覺六批；Web 家屬導覽補上 persona 傳遞，App timer 採跨平台明確型別。 |
