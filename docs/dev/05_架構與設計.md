...
> **版本:** v1.19 | **更新:** 2026-07-19 | **狀態:** ✅ 定稿（深化完成；庚批 2026-07-13 結案回填；locations／strategies／自適應問候時間已入圖；§8.5 補主動問候接續上次話題；新增 §12 工程觀測 Opik 雙視角）
...
**接線點**：`composition.build_externals` 呼叫 `tracing.configure` 並以 `wrap_genai` 包所有 Gemini client；`pipeline.process/process_text` 為 trace root，`_transcribe/_assess/_generate/_synthesize` 為 span；`tools/registry.dispatch`、`rag/retriever.retrieve` 各為 span。
