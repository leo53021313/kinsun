...
> **版本:** v1.13 | **更新:** 2026-07-17 | **狀態:** ✅ 定稿（深化完成；庚批 2026-07-13 結案回填；locations／strategies／自適應問候時間已入圖；§8.5 補主動問候接續昨天話題）
...
**關鍵流程（App 語音回合 `POST /api/v1/turns`）**：token 驗證→同意複核（403 守門，`turns.py:92`）→**寫入長輩地點**（地名＋模糊座標三者齊備才寫，排在 dispatch 之前，2026-07-17）→進站音檔託管→`InboundMessage(APP)`→`dispatch`（App 路徑傳入已解析 elder_id 跳過重查，✅ 庚-12）→`VoicePipeline.process`→回覆音檔簽章 URL→`record_reply`（round_trip_ms）。共 8 個分岔：403 同意撤回／415 非音檔／413 超大／未綁定提示／危急 L2＋落庫並通知家屬／TTS 失敗退純文字／管線失敗 FALLBACK_PROMPT／音檔上傳失敗退純文字。
...
**接續昨天話題（spec 2026-07-17-主動問候接續昨天話題）**：兩種主動推播送出前，`worker._yesterday_recall` 先讀昨天的對話摘要（`summaries.get_for_date`），經 `agent.proactive(..., recall=)` 一物二用——既取代 intent 當長期記憶的**檢索關鍵字**，也直接注入 prompt 給模型看。動機：intent 是每天每位長輩都相同的字串，拿它做語意檢索等於每天用同一把萬用鑰匙開所有人的門，撈回的與她昨天講什麼無關，問候長期收斂到 `HEALTH_QUERY` 撈出的健康罐頭；而早上 `recent()` 必為空（只回今日），昨天的事只能靠這條路進來。為何是摘要而非 Mem0 的昨晚整理項：後者的抽取指示明訂「閒聊與情緒由系統另行處理」，裝的正是罐頭本身，且凌晨 3 點寫入使 `created_at` 記今天、內容是昨天，日期對不上。`recall=None`（昨天沒講話／讀取失敗）＝一字不差退回本功能之前的行為——摘要是錦上添花，不可擋下問候。
...
| v1.13 | 2026-07-17 | §8.5 新增「接續昨天話題」段（辛-9）：主動推播送出前讀昨天對話摘要，經 `proactive(recall=)` 兼作長期記憶檢索關鍵字與直接注入；載明為何不用 Mem0 昨晚整理項（抽取指示排除閒聊＋寫入日與內容日錯開）與 `recall=None` 的回退語意 |
