...
| 網頁版前端（web/） | 瀏覽器 runtime | React＋Vite（`web/src/`，由 API 進程託管靜態檔） | 現行（P1 地基＋P2 家屬端＋P3 對講機協定核心＋錄音播放與 iOS 音訊解鎖＋長輩端 QR 掃碼＋長輩端 API 呼叫端與文案＋長輩配對與帳密重登畫面皆已完成；對講機與提醒兩個長輩端畫面待 Task 8／9；App／LIFF 凍結後唯一演進的前端，spec 2026-07-30） | 略：見 12／17 |
