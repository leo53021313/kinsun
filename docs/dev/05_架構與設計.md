...
> **版本:** v1.20 | **更新:** 2026-07-19 | **狀態:** ✅ 定稿（深化完成；庚批 2026-07-13 結案回填；locations／strategies／自適應問候時間已入圖；§8.5 補主動問候接續上次話題；§12 工程觀測 Opik 雙視角——全鏈路 span 盤點＋排程/批次 trace root）
...
flowchart TD
    subgraph container["Container: API 服務"]
        ...
        subgraph orch["應用編排層"]
            ...
            tools["tools〔群1/2b〕<br/>ToolRegistry：weather｜clock｜health_rag｜web_search"]
...
flowchart TD
    subgraph container["Container: 排程 Worker"]
        ...
        jobs["jobs 清單<br/>用藥×4／回診／問候（自適應時刻）／失聯／<br/>夜間批次（整理→摘要→反思→問候時間）／清理×2"]
...
| CareAgent | 記憶組裝→LLM→工具迴圈（≤3 輪，handle 與 proactive 皆可用工具）→出站語音安全防線（JSON／fence 打撈，2026-07-17）→寫回（`agent.py`） | memory、llm、tools |
...
36 表全清單見 `db.py`（2026-07-17 重數）；此處畫身分／照護核心。獨立群組文字帶過：觀測五表（webhook_events、asr_calls、llm_calls、tts_calls、replies——external_id＋channel 正名，✅ 庚-07）、RAG 四表（rag_sources／documents／chunks／ingestion_audit_logs；crawl_jobs 已退役 庚-34）、守則兩表（strategies／web_search_lookups）、狀態表（scheduler_state／rate_limit_hits／memory_consolidations）。
...
| :--- | :--- | :--- |
| 日誌 | logs/*.log（每服務一檔） | ✅ |
| 追蹤 | 自建五階段 trace（webhook→asr→llm→tts→reply，trace_id 貫穿；`get_trace` 為應用層 trace_id 匯聚非 JOIN）＋admin 後台（2026-07-12 內測強化 D-73：長輩詳情五分頁＋系統排程頁＋內測手動觸發） | ✅ token 量測已落地（✅ D-05 戊-2）；安全分級 LLM 亦已入收集範圍（✅ 庚-10 A-9：`_assess` 補 llm_call trace，每輪 llm_calls 2 筆） |
...
| tools | `ToolRegistry`（register／specs／dispatch 永不拋）；組裝處註冊 4 工具：get_weather、get_current_time、health_education_rag、web_search。天氣工具防 anchoring（2026-07-17）：只信長輩原話（`turn_context.elder_utterance` contextvar）或注入座標，拒答模型臆測地名；地理編碼限台灣 | `ToolSpec` |
...
**跨群介面**：←群 1（`ToolRegistry` 註冊 `HEALTH_RAG_SPEC`，工具失敗不中斷對話）；←群 4（安全升級目前僅 advisory——A-27）；→群 6（db `rag_*` 表、`RAG_TOP_K` 經 config；D-70 鍵除外）。

**缺口處置（2026-07-13 庚批結案）**：A-26 著作權（庚-03）與 A-28／A-29／A-30（庚-21～23）經 Leo 決議**擱置**（RAG 先不處理）；A-31 死碼已清（✅ 庚-34：InMemoryVectorStore／document_loader 刪除、rag_crawl_jobs 退役）；A-33 已修（✅ 庚-39）；**A-27 升級旗標仍未接線（庚-09 未結）**。
...
| A-40 | ~~核心承諾無失敗感知~~ ✅ **已修復**（庚-02，2026-07-12：admin 紅字橫幅；重試／死信不做——Leo 拍板最小方案） | 可靠性→已修 |
...
- `tracing/client.py`：`configure`／`is_enabled`，`OPIK_ENABLED` 全域開關；停用或未裝 opik 時恆 no-op。
...
- 排程/批次各自為 root：`agent.proactive`（`proactive_turn`）、`strategies/reflection.reflect_days`（`nightly_reflection`）、`reports/summaries.summarize_day`（`daily_summary`）、`memory/longterm/consolidation.run_consolidation`（`memory_consolidation`）、`rag/ingest.main`（`rag_ingest`）。
...
**橫切增強**：`tag_current_trace` 掛 `kinsun_trace_id`＋以 `elder_id` 設 `thread_id`（同一長輩多回合串成對話串，E1）；`update_trace_metadata` 追加 `risk_tier`／`fallback`／`flow`（E4）；`log_feedback_score` 供家屬評分／線上規則回掛（E2）。成本/token 由 track_genai＋既有 `LLMUsage` 收集（E5）。線上評測規則於 Opik UI 設定，無 repo 程式碼（E3）。

**安全**：任何 `api_key`／音檔 bytes／向量一律不進 capture；觀測失敗一律吞掉不中斷對話。

**相依方向（單向、可插拔）**：`llm／pipeline／tools／rag／evals → kinsun.tracing → opik SDK`；`observability/` 不依賴 `tracing/`。

**位置無關**：kinsun 只透過 `OPIK_URL_OVERRIDE` 連 Opik，搬遷只改環境變數、程式碼不動。Opik 服務本身不進版控（外部基礎設施，見 [14 §Opik 服務](14_部署與運維.md)）。

**離線評測**：`evals/`（頂層，與 tests/ 平行）以 Opik dataset＋LLM-judge 指標（Hallucination 等）做防幻覺離線關卡，不在請求路徑。
