...
全表清單見 `db.py`；此處畫身分／照護核心。獨立群組包含觀測六表（webhook_events、asr_calls、llm_calls、rag_calls、tts_calls、replies）、RAG 的 sources／documents／chunks／ingestion audit／index releases／release documents／release chunks、守則與 web search 查詢紀錄、話題新聞 news_items（決定性 id upsert，D-74）與提及紀錄 news_mentions（不重複給料，D-74 消費端）、附近地點 `places`（Overture Maps 匯入的台灣店家 POI，非長輩擁有的全域查詢表，spec 2026-07-27；`place_id` 為 Overture 原生 id，`ingest.py` 據此 upsert），以及 scheduler／rate limit／memory consolidation 狀態表。
...
| 失敗點 | 使用者得到 | 有聲音？ |
| :--- | :--- | :---: |
| ASR／LLM／記憶失敗 | `FALLBACK_PROMPT`＝`SYSTEM_TROUBLE_REPLY`「我這邊有點小狀況…」（`inbound.py:26`） | ✅ **預錄語音**（V-02 已補，2026-07-29；取不到音檔才退純文字） |
...
### 8.8 Expo App 阿白角色 renderer（2026-08-07，批次 1）

- **邊界**：`app/src/components/OttoBearRenderer.tsx:38` 是純呈現元件；正式 `src/kinsun/agent.py:61` 仍是角色回覆與安全提示的唯一執行來源。vendored `brain.js` 只留上游人設參考，不進 renderer 產物；Otto 的第二套麥克風、LLM、TTS、定位與外部網路模組均不納入 runtime。
- **接縫**：`app/src/lib/ottoBridge.ts:28` 建立版本化 `sync` 訊息，只容許既有 `idle／listening／thinking／speaking／error` 五態。`talk.tsx` 在每段音訊真正開始播放時才建立 `speechCue`（WebSocket：`app/src/app/elder/talk.tsx:195`；POST：`:367`；續段：`:466`），因此第一段抵達即可驅動角色，不等待整段 TTS 完成；三支狀態機未修改。
- **安全**：renderer 由 `app/scripts/build-otto-renderer.mjs:8` 白名單組裝，HTML 以 `default-src 'none'` CSP 封住網路（`renderer.template.html:6`）。`sentiment.js:97` 在本地關鍵詞完成命中後再套固定黑名單，LLM 結構化情緒亦走同一道最後防線；回應情緒只換角色本身，不回寫系統態。
- **降級**：原生 WebView 尚未 ready 或載入失敗時保留既有靜態暫用圖；Expo Web 因 `react-native-webview` 不支援瀏覽器，同樣使用靜態圖。這不影響錄音、分段播放或後端回覆。

### 8.9 Expo App 對講機呈現與本機當日紀錄（2026-08-07，批次 3）

- **固定呈現邊界**：`BearStage` 取代舊 `AvatarPlaceholder`，只接既有 `TalkVisualState`、回應情緒與對嘴 cue；舞台固定 top 140、209×300。`talk.tsx` 的頁首、舞台、內容層與主鍵分成四個絕對定位層，內容增加時不改角色位置；系統字級達 150% 才開內容層捲動。
- **延遲邊界**：後端仍先合成第一段；App 收到第一個 `reply` frame 就展開回話卡、顯示該 frame 已帶的完整回答文字並開始第一段對嘴，後續 `chunk` 只補分段音訊。`collapsed`、完整文字顯示與 132→72 主鍵動畫皆在呈現層，不新增第六個視覺狀態，也不修改 `talkGesture`／`talkSocket`／`talkPresentation`。
- **資料與隱私邊界**：長輩成功回應新增 `transcript`，REST 與 WS 皆只回該長輩的 bearer／socket 連線。`todayLog` 把真實「您說」與完整阿白回答寫進長輩裝置 AsyncStorage，只留本地自然日、登出清除；家屬通知、摘要與清單 API 不新增逐字內容。
- **競態邊界**：同一 WS 可有三輪在途，`todayLog` 以單一 Promise 鏈序列化 read-modify-write；每輪以 `turn_id` 保留 transcript、`reply` 的完整文字與待播音檔數，`chunk.text` 不再串入完整回答，最後一段播完才收合與落本機紀錄。空白 `is_last=true` 訊框只完成協定，不進播放器。

### 8.10 Expo App「之前聊過的」當日紀錄畫面（2026-08-07，批次 4）

- **資料邊界**：`app/src/app/elder/history.tsx` 只在長輩 session 下讀取 `todayLog.loadToday()`，不呼叫後端，也不把逐字內容交給家屬端模組。跨日仍由 `todayLog` 自然回空；本批沒有新增 API、資料表或環境變數。
- **導航邊界**：`/elder/talk` 的 60dp 頁首入口 push 到 `/elder/history`；返回優先使用既有 Stack，直接由深層連結進入而沒有返回堆疊時固定導回 `/elder/talk`，不導到 `/role`。
- **呈現邊界**：頁首在 `ScrollView` 外固定，只有內容清單可在溢出時捲動。紀錄反轉副本後最新一輪在最上方；所有文字至少 22px 且不設 `maxFontSizeMultiplier`。
- **重播邊界**：本機紀錄沒有可持久化的音檔識別與重新合成契約，故依 2026-08-07 使用者選項 1，暫不渲染「再聽一次」；不留下空函式或無作用按鈕。

### 8.11 Expo App 家屬端 Tabs 與外層深頁（2026-08-07，批次 5）

- **路由邊界**：`app/src/app/guardian/_layout.tsx` 只承載 home／report／add／notifications／profile 五個底部分頁；登入、註冊移到 `auth/`，長輩詳情與行程管理移到 `guardian-detail/`，由根 Stack 顯示且不帶 tab bar。未登入或角色不符時，Tabs 掛載前即導回 `/role`。
- **選取邊界**：交付稿尚未定義多位長輩切換器，依 2026-08-07 使用者裁決，`GuardianTabsProvider` 暫以 `listElders()` 第一位為目前長輩。規則集中一處；無長輩時回首頁，report／profile 只接既有詳情資料，不在批次 5 發明批次 6 欄位。
- **導航邊界**：中央暖黃「新增提醒」以 `tabPress.preventDefault()` 攔截，重讀第一位長輩後 `push` 外層行程頁，不把 add 留在歷史堆疊。Android 實體返回鍵在非首頁 tab 先回 home；home 維持系統預設退出行為。所有深頁使用 `push`，保留返回原 tab 的堆疊。
- **未讀邊界**：通知頁以 `lazy:false` 預載後自行取得 session、讀通知與已讀水位，再透過 `navigation.setOptions()` 設 badge；進入通知頁以最新 `created_at` 更新水位並清 badge。layout 不承擔通知資料責任。
- **變更面**：本批未新增 API、資料表、環境變數或第三方套件；只重整 Expo Router 邊界與家屬端導航。批次 6 的摘要、改回診與危急詳情畫面尚未開始。
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.81 | 2026-08-07 | 新增 §8.11：App 批次 5 將家屬端改為五項 Tabs，認證與深頁移到根 Stack；集中第一位長輩暫用規則、Android 返回與通知頁自行設定 badge 的邊界。未新增 API、Schema 或套件，批次 6 未開始。 |
| v1.80 | 2026-08-07 | 新增 §8.10：App 批次 4 `/elder/history` 只讀本機當日紀錄，固定頁首／內容層捲動、最新在上、session 與返回邊界完成；依使用者選項 1 暫不顯示未具真實重播契約的按鈕。 |
| v1.79 | 2026-08-07 | 新增 §8.9：Expo App 批次 3 固定對講舞台、第一段即展開、呈現層收合與主鍵動畫；新增長輩本人 `transcript` 回傳及 AsyncStorage 當日紀錄的資料／隱私／併發邊界，三支既有狀態機不動。 |
| v1.78 | 2026-08-07 | 新增 §8.8 與 ADR-013 對齊：Expo App 以離線 WebView 接入阿白角色 renderer；正式後端維持唯一對話與安全來源，bridge 只收五態、分段文字與 `duration_ms`，第一段開始播放即驅動，三支狀態機不動；LLM prompt 與兩句確定性語音回退統一為阿白第一人稱。 |
