...
> **版本:** v1.8 | **更新:** 2026-07-10 | **狀態:** ✅ 定稿（架構文檔深化完成：§8 七節全補齊＋C4 Component 圖重畫＋一致性檢查通過）
...
flowchart TD
    subgraph container["Container: API 服務"]
        ...
        subgraph orch["應用編排層"]
            ...
            tools["tools〔群1/2b〕<br/>ToolRegistry：weather｜clock｜health_rag"]
        ...
        subgraph domain["領域層"]
            ...
            meds["medications〔群3〕"]
            appts["appointments〔群3〕"]
            reports["reports〔群3〕<br/>health｜reminders｜summaries"]
            safety["safety〔群4〕<br/>detector＋classifier＋events＋deliveries"]
...
| :--- | :--- | :--- |
| 日誌 | logs/*.log（每服務一檔） | ✅ |
| 追蹤 | 自建五階段 trace（webhook→asr→llm→tts→reply，trace_id 貫穿；`get_trace` 為應用層 trace_id 匯聚非 JOIN）＋admin 後台 | ✅ token 量測已落地（✅ D-05 戊-2：`collect_llm_usage` contextvars 收集→`llm_calls`→overview SUM；假 LLM 無 usage 才記 NULL）。⚠ 殘缺：安全分級 LLM 在收集範圍外（A-9） |
...
| Phase | 範圍與目標 |
| :--- | :--- |
| Phase 1（→8/20） | 一次性重構（16_WBS）：**已完成**——D-11 文字管線、D-12 App 出站與通知頁、D-18／D-19 分層修正、KPI 量測基建（token＋四階段延遲）。**剩餘**：多 worker（D-20，前置 A-54／A-55 未清）＋本次深潛新增的 A-8～A-58 待排序（六項 HIGH 優先） |
...
fail-safe 三層防護（classifier 捕 LLMError→detector 捕全例外→收斂 L1）；舊庫 tier=3 由 `tier_from_db`＋classifier 雙保險夾回 L2。詞表仍為 placeholder（絕對 6＋症狀 5 詞，A-43）。

**危急通知鏈**：pipeline **先落庫再通知**（agent 例外不吞危急）→ `GuardianNotifier` 全家屬齊發無冷卻（✅ D-10）→ `TextSender` 插座（✅ D-18）→ `ChannelRouter` 逐通道扇出。每家屬送達成敗記 `risk_notification_logs`（✅ D-36，留痕失敗不反噬通知）；`keyword:absolute` 訊號才掛 119 提示。`escalation_order` 已從升級語意退化為排序語意（升級鏈不做，己-8 保留欄位）。⚠ 可靠性缺口：**無重試、無死信、無失敗告警**——admin 告警只涵蓋分級器故障，不涵蓋「L2 通知家屬失敗」（A-40 HIGH）；App 通道落庫即記 delivered=True，高估真實送達（A-41）。

**⚠ L1 小訊號斷點（A-39 HIGH）**：`risk_events` 唯一寫入閘門＝「L2 以上或 fail-safe」，一般 L1 刻意不落庫（`test_plain_l1_not_recorded` 明確斷言）；每日摘要的 L1 注入卻排除 fail-safe——交集為空，D-10 己-5「L1 小訊號進每日摘要」生產環境永不觸發。

**主動關懷**：早安問候（無條件）＋失聯關心（`last_active` ≥ 2 天，只計 user turn——主動關懷寫 assistant turn 不自我抑制，刻意設計勿誤改）；先 `has_route` 確認可達才花 LLM；proactive 不跑工具迴圈；記憶只記 assistant 單輪帶【主動關懷｜intent】標記（✅ D-39）。

**scheduler**：自建輕量排程（croniter＋`scheduler_state` upsert）；`_is_due` 首見種基準、停機補跑一次（不逐日回補，A-18 同源）；fanout 逐筆隔離無重試（單筆失敗當日遺失）；**單 worker 假設無鎖**（誤起雙實例會重複提醒，A-42）。worker 組裝 11 個 job：整理／問候／失聯／用藥×4／回診／音檔清理×2／觀測清理。
...
**跨群介面**：←群 1（pipeline 同步呼叫 assess／notify；TextSender＝ChannelRouter）；→群 2a（`sessions()` 為 fanout 母體、`last_active` 供失聯、proactive 寫記憶）；→群 3（reminder_logs 記錄、summaries 讀 L1——現斷線 A-39）；→群 5（`guardians_of`／escalation_order）；→群 6（db 四表、config `SAFETY_*`／`PROACTIVE_*`、admin 告警）。
...
| App 帳密 | `guardian_accounts`（email）／`elder_accounts`（phone，✅ 己-6 D-71） | scrypt 雜湊（N=16384/r=8/p=1，參數隨值保存） |
...
| 認證 | 發放 | 驗證 | 撤銷 |
| :--- | :--- | :--- | :--- |
| 裝置 token（長輩） | 綁定碼兌換或帳密重登 | `authenticate_token`（SHA-256 查表） | 僅家屬 `revoke_elder_device`（撤全部 token＋拆 App 綁定）——長輩無自助登出（A-49） |
...
**一致性邊界**：`AccountService` 是 Elder 聚合的唯一維護者，跨表原子寫入靠 `store.transaction()` seam（`create_elder`／`redeem_invite`／`register_guardian_account` 皆包交易）。⚠ 三處多筆寫入**未包交易**：`revoke_elder_device`、`login_elder`／`login_guardian` 的補綁定＋發 token（A-48）；`FakeAccountStore.transaction` 為 no-op，合約測試抓不到交易語意。

**已知安全缺口**：`bind_elder_device` 不檢查 `invite.role`（家屬邀請碼可換出長輩 token，A-46 HIGH）；家屬 token 無批次撤銷（永久 token 外洩無「登出所有裝置」，A-47 HIGH）；邀請碼 redeem 為 TOCTOU 無列鎖（A-49）；長輩憑證＝低熵手機號＋8 字元下限（A-50）。
...
| `assemble_core(settings, externals, *, clock)` | 15 欄位 frozen `Core`：帳號／用藥／回診／記憶／SessionMemory／RAG／CareAgent＋ToolRegistry／ChannelRouter／traces／reminder_logs／notifications | 純接線、可注入假 Externals **離線測** |
| 兩組裝根 edge-specific | `build_app`：VoicePipeline、RiskDetector、GuardianNotifier、Gate、ratelimit、4 個 router、靜態掛載；`build_scheduler`：Scheduler＋11 個 job | root-specific 不進 Core |

「禁止 handler 內自行 new」實測**零例外**；`CareAgent` 只在 composition 建構（有結構守門測試）。⚠ `PgRiskEventStore`／`PgConversationSummaryStore` 兩根各 new 一次卻未進 Core（同類的 traces 卻在 Core 內）——邊界不一致（A-57）。

**config**：`Settings` frozen、55 欄位、**欄位名＝環境變數小寫 100% 對應無別名**；載入序＝真實環境變數 > `.env` > 內建預設；必填 4 項（`LINE_CHANNEL_SECRET`／`LINE_CHANNEL_ACCESS_TOKEN`／`GEMINI_API_KEY`／`DATABASE_URL`）缺即 `ConfigError` **啟動即死**（fail-fast，無延後爆）；程式讀的 55 鍵全數列於 `.env.example`（合規）。

**db**：`Executor` Protocol（execute／query／query_one）＋`_Errors` 例外轉譯層（底層 → `StoreError` → 各領域 Error）；`ensure_schema` 共 20 段（15 建表＋5 冪等遷移），開頭以 `pg_advisory_xact_lock` 串行化——webhook 與 scheduler 同時啟動不會 DDL 死結（有整合測試）。

**信封契約**（✅ D-23／D-24）：`{success, data, error{code,message}, meta}` 與 `shared/envelope.ts` **逐欄一致**；後端把 code 譯成繁中人話，UI 直接顯示。⚠ 錯誤碼本身是散落各 handler 的字串字面值、無中央 enum，且 `ERROR_MESSAGES` 已與 06 §3 表脫節（A-56）。

**可觀測性**：五表 append-only，一律經 `safe_record` 包裹（**觀測失敗不中斷對話**）；`get_trace` 為**應用層 trace_id 匯聚**（六表各查一次再於 Python 組裝，非 SQL JOIN）＋`channel_bindings JOIN elders` 解析姓名；`get_overview_stats` 統計 token 總量與四階段 p50／p95；清理 job（03:45）`purge_older_than` **只刪五表**，`risk_events`／`turns` 永久保留。

**節流與安全**：`SlidingWindowRateLimiter` per-IP 滑動視窗（被擋的嘗試不計數），4 個 scope（register／login／**elder-login**／bind）共用同一 limiter 與同一組參數；⚠ **記憶體版，多 worker 下實際上限＝設定值×worker 數**（D-20 上線後安全放大，A-54 HIGH）。`security.py` 掛 5 個標頭（HSTS／nosniff／DENY／no-referrer／CSP）；admin `X-Admin-Key` 以 `hmac.compare_digest` 常數時間比對＋金鑰未設 fail-closed 503。

**NFR 實現**：可測性＝Core 離線可測＋composition 結構守門測試；可靠性＝觀測 fail-open、DDL advisory lock；安全＝fail-fast 設定、常數時間金鑰比對、5 標頭；**可擴展（D-20）前置未清**：ratelimit per-process（A-54）＋連線池總量未評估（A-55）。
...
### 彙整：2026-07-10 架構文檔深化的六項 HIGH（建議優先排序）

| # | 一句話 | 性質 |
| :--- | :--- | :--- |
| A-39 | **已拍板功能靜默失效**：D-10 己-5「L1 小訊號進每日摘要」在生產路徑永不觸發（落庫閘門與摘要過濾交集為空） | 功能死亡 |
| A-40 | **核心承諾無失敗感知**：危急通知失敗無重試、無死信、無告警（admin 只看分級器故障） | 可靠性 |
| A-26 | **著作權合規**：SourceValidator 不查 copyright，兩個 DISALLOWED 醫院來源在預設 ingest 名單內 | 合規 |
| A-46 | **權限邊界**：`bind_elder_device` 不驗 `invite.role`，家屬邀請碼可換出長輩裝置 token | 安全 |
| A-47 | **憑證撤銷不對稱**：家屬永久 token 無「登出所有裝置」批次撤銷 | 安全 |
| A-8 | **命名鐵律違反**：觀測五表 `line_user_id` 欄位承載所有通道的 external_id | 資料模型 |
| A-54 | **多 worker 前置**：ratelimit per-process，D-20 上線後登入節流上限×N（長輩低熵手機號帳號同池） | 安全／擴展 |

> 另有 A-18（停機多日→記憶永久漏整理）屬 HIGH 級資料遺失風險，與 A-42（scheduler 無鎖）同源於排程設計，建議合併評估。完整 58 項見下表；群 7 前端發現另列 [12 §9](12_前端架構規範.md)。
...
| A-39 | **（HIGH）** L1 小訊號斷點：pipeline 落庫閘門（L2 或 fail-safe）×摘要注入過濾（排除 fail-safe）交集為空——D-10 己-5「L1 小訊號進每日摘要」生產環境永不觸發；兩半各自測試通過掩蓋斷線（`test_plain_l1_not_recorded` 刻意斷言不落庫）。修法：pipeline 對一般 L1 也落庫（通知閘門維持 L2），或明文降級承諾並更新 D-10／BDD（`pipeline.py:85-92`×`summaries.py:95-101`） | 群 4 深潛 2026-07-10 |
...
| A-46 | **（HIGH·安全）** `bind_elder_device` 不檢查 `invite.role`：GUARDIAN 邀請碼 POST `/device-bindings` 會走 guardian 分支（建空名 Guardian＋消耗邀請碼）卻仍發出 **ELDER 裝置 token**。緩解：guardian 分支不寫 consent→對講機被 ConsentGate 擋 403。修法：redeem 前先驗 `invite.role == ELDER`（`service.py:321-327`） | 群 5 深潛 2026-07-10 |
| A-47 | **（HIGH·安全）** 家屬 token 無批次撤銷：`logout` 只撤當前一顆（需持有 token），`remove_api_tokens_for_principal` 已實作卻無家屬端點呼叫——搭配 D-25 永久 token，外洩後無「登出所有裝置」手段（長輩有 `revoke_elder_device`，兩者不對稱）（`service.py:332-334`） | 群 5 深潛 2026-07-10 |
