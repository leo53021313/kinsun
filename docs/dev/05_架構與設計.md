...
| :--- | :--- | :--- |
| 日誌 | logs/*.log（每服務一檔） | ✅ |
| 追蹤 | 自建五階段 trace（webhook→asr→llm→tts→reply，trace_id 貫穿；`get_trace` 為應用層 trace_id 匯聚非 JOIN）＋admin 後台（2026-07-12 內測強化 D-73：長輩詳情五分頁＋系統排程頁＋內測手動觸發） | ✅ token 量測已落地（✅ D-05 戊-2：`collect_llm_usage` contextvars 收集→`llm_calls`→overview SUM；假 LLM 無 usage 才記 NULL）。⚠ 殘缺：安全分級 LLM 在收集範圍外（A-9） |
...
| `assemble_core(settings, externals, *, clock)` | 16 欄位 frozen `Core`：帳號／用藥／回診／記憶／SessionMemory／RAG／CareAgent＋ToolRegistry／ChannelRouter／traces／reminder_logs／notifications（2026-07-12 增 `account_store` 供 admin 觀測直查，D-73） | 純接線、可注入假 Externals **離線測** |
...
**config**：`Settings` frozen、56 欄位（2026-07-12 增 `internal_testing_enabled`，D-73）、**欄位名＝環境變數小寫 100% 對應無別名**；載入序＝真實環境變數 > `.env` > 內建預設；必填 4 項（`LINE_CHANNEL_SECRET`／`LINE_CHANNEL_ACCESS_TOKEN`／`GEMINI_API_KEY`／`DATABASE_URL`）缺即 `ConfigError` **啟動即死**（fail-fast，無延後爆）；程式讀的 56 鍵全數列於 `.env.example`（合規）。
