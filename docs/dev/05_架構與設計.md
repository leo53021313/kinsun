...
**關鍵流程（App 語音回合 `POST /api/v1/turns`）**：token 驗證→同意複核（403 守門，`turns.py:92`）→**寫入長輩地點**（地名＋模糊座標三者齊備才寫，排在 dispatch 之前，2026-07-17）→**啟動**進站音檔背景上傳（不等它，2026-07-30 延遲優化 B1）→`InboundMessage(APP)`→`dispatch`（App 路徑傳入已解析 elder_id 跳過重查，✅ 庚-12）→`VoicePipeline.process`→回覆音檔簽章 URL→`record_reply`（round_trip_ms）。共 8 個分岔：403 同意撤回／415 非音檔／413 超大／未綁定提示／危急 L2＋落庫並通知家屬／TTS 失敗退純文字／管線失敗 FALLBACK_PROMPT／ASR 辨識為空或純標點幻覺（靜音誤觸——DGX 端靜音峰值閘回空，或 Whisper 系對近無聲短檔幻覺出純標點「? ? ?」，管線 `_has_recognizable_speech` 去標點後判空）→ 跳過分級與生成、以 FALLBACK_REPLY 合成語音請長輩再說一次（2026-07-18）。
