...
**併發輪的四道安全（P3）**：①`turns` 以**長輩開口的時刻**（`PreparedTurn.spoke_at` → `record_turn(at=…)`）為排序鍵，否則併發輪的對話順序會顛倒、user／assistant 還可能交錯；②在途清單（連線層，記憶體內）讓第二句的指代（「**那**天氣呢」）看得到還沒寫進 DB 的第一句——刻意不採「先寫 DB 後補」，那會違反「被濫用審核攔下的輪不寫進記憶」的安全契約；③同時在跑的輪數上限 3，超過婉拒且**講出來**；④晚到的答案以系統提示要求模型開頭自然帶一句回指。三個跨層訊號（`tool_announcer`／`transcript_listener`／`pending_utterances`／`turn_directive`）沿用 `turn_context` 既有的 contextvars 機制，沒有人在聽時全部 no-op，故 LINE、`POST /turns` 與排程端行為不變。
