...
| voice_profiles | 長輩客製化聲音複製（聲音克隆，2026-07-30）：`VoiceProfileStore` 依 `elder_id` 存一組參考音檔＋逐字稿＋同意紀錄；`VoicePipeline._synthesize` 查出生效中設定檔即組 `VoiceReference` 傳給 TTS，無設定檔則沿用 DGX 端全域預設聲音 | `VoiceProfile`、`VoiceProfileStore` |
| services（DGX） | `POST /transcribe`（bytes→`{"text"}`）、`POST /synthesize`（`{"text", elder_id?, prompt_audio_url?, prompt_text?}`→m4a＋`X-Duration-Ms`；後三欄位選填＝長輩客製化聲音，缺 elder_id 或無法解析即退回全域預設聲音）；併發閘滿回 503 | — |
...
| 元件 | 職責 | 關鍵設計 |
| :--- | :--- | :--- |
| `SessionMemory`（recall.py） | 三層門面：`assemble(elder_id, query)→TurnContext`、`record_turn` | 事實層 query 無關（保證每輪帶）；單一 FactProvider 失敗略過不中斷（`recall.py:52-56`）；`TimeFacts` 排首位（2026-07-25，`clock.py`）——時間是其他事實的座標系（回診印絕對日期），且段首寫死「沒問就不用特地報時」防模型逢句報時（同 LocationFacts 的 anchoring 教訓） |
