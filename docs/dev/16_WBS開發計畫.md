...
| 辛-30 | 🟡 **網頁版（`web/`）補齊新視覺六批**（使用者 2026-08-09 裁決：發表要用網頁做 demo，兩端視覺不可分岔）。辛-28 的六批交接從頭到尾只指名 `RAG/app/`，`web/` 完整停在改版前：色票仍是珊瑚橘、`strings.ts` 30 處把角色叫「金孫」、`elder/Avatar.tsx` 是 emoji、家屬端仍是 `useScreenStack` 無 Tabs、缺「之前聊過的」與批次 6 三支畫面。分六步對齊：**W1 token 與文案**、W2 共用元件、W3 對講機呈現層（阿白改接 Otto renderer——`pet-core` 本就是 web 技術，掛 DOM 不需 WebView）、W4 之前聊過的、W5 家屬端 Tabs、W6 家屬端新畫面。✅ **W1 token 完成**——`theme.css` 9→17 色＋五態光暈／狀態帶、舞台幾何、長輩端觸控尺寸、動畫時長與陰影；`theme.test.ts` 由「只驗 token 名字」改為兩層，新增一層直接讀 `app/src/lib/theme.ts` 逐一比對色值（色碼不寫死在測試裡，美術迭代仍只改 app 端那一份）——這正是本次漏掉的失敗型態：檔頭宣稱「值與 app 端完全相同」而 app 換色後 web 沒跟上，沒有任何測試會紅。41→55 條，`tsc`／629 條測試／`npm run build` 全過。✅ **W1 文案完成**——`strings.ts` 30 處「金孫」逐條分類：21 處改為阿白（長輩端自述用第一人稱「我」、狀態 aria-label 與家屬端用第三人稱「阿白」），10 處保留為服務名（品牌字樣、同意條款主體、「連不上金孫」、「用過金孫？」、服務狀態頁的 ASR／TTS 分項說明，以及「長輩手機上的金孫會被登出」——那個金孫是 App 本身）。新增 `strings.test.ts` 30 條，分兩組列舉：該叫阿白的地方不准殘留服務名、該是服務名的地方必須還在（不可用「全庫不准出現金孫」一條斷言了事，那正是鐵律 8 警告的錯誤做法）。連帶更新 7 支既有測試共 27 處寫死的舊文案期望值，並校正一處停在舊版的後端婉拒話 fixture（`ws.py::_BUSY_REPLY` 早已是第一人稱）。`tsc`／eslint 乾淨，42 檔 659 條測試全過，build 通過。✅ **W2 共用元件完成**——`ui/Button` 改暖黃膠囊配深靛藍字（舊版是磚橘底白字）、四 variant、56／84 最小高、按下縮放、busy 保留動作名稱、focus 2px 描邊＋2px 外偏移；`Field` 加 hint 並把家屬端標籤由 14px／textSoft 提到 16px／ink；`Section` 加 inset；`ErrorText` 改用 dangerText（鐵律 15）；新增 `StatusPill`（五 tone 三重編碼）與 `Chip`（硬下限 48px，選取狀態用 `aria-checked` 而非 `aria-selected`——app 端那支從交付稿繼承了 `selected`，web 直接做對）。**新元件當場接上呼叫端**，不留「寫了沒人用」：`SchedulesScreen` 兩組自寫 chip（提醒類型 radio、用藥時段 checkbox）改用共用 `Chip`，`InviteCard` 的綁定碼面板改用 `Section inset`。ui 測試 8→33 條，42 檔 675 條全過。✅ **W3a 接上阿白完成**（W3 拆兩步：W3a 只換角色、W3b 才動版面，出錯時分得出是渲染還是版面）——`elder/Avatar.tsx` 的 emoji 換成 `elder/BearStage.tsx`，以 iframe 載入與 App **同一份** `renderer.html`；`ottoBridge` 協定與產生器搬進 `shared/`，產生器同時寫出 `app/assets/otto/` 與 `web/public/otto/` 兩份產物、`--check` 兩份都驗；`kinsun-bridge.js` 的 `notify` 補瀏覽器 iframe 回傳路徑（原本只認 `ReactNativeWebView`）。刻意**不做**靜態暫用圖降級——App 那側退回的是舊角色阿金，網頁版不重複。`role="img"`＋aria-label 保留：網頁版狀態帶要到 W3b 才有，先拿掉會讓視障長輩失去唯一線索。新增 `BearStage.test.tsx` 8 條驗 bridge 接線（來源驗證、ready 後補送、sequence 遞增、sandbox 不給 allow-same-origin），43 檔 683 條全過。✅ **手機外框改為真機尺寸**（使用者 2026-08-09 指定「最新 iPhone」）——`stage/PhoneFrame.tsx` 由 `aspect-[9/19.5]`＋`max-w-[380px]` 的通用直式比例改成 iPhone 17／17 Pro 的實際視埠 **402 × 874 CSS px**（兩機型視埠相同，同時涵蓋兩種讀法；17 Pro Max 440 × 956、Air 420 × 912 只改兩個常數即可切換）。邊框改用 `outline` 而非 `border`——border-box 下 10px 邊框會從 402 裡吃掉、螢幕實際只剩 382，量出來的位置全部偏移。狀態列 54px、home indicator 34px、Dynamic Island 125 × 37 均照真機安全區，內容區因此拿到的高度就是真機上 app 可用的高度——這是 W3b「一屏不捲」能不能驗的前提。✅ **W3b 對講機版面完成**——`TalkScreen` 由 flex 直欄改成四層絕對定位（頁首 z40／舞台 z10／內容層 z20 貼底／主鍵 z30），容器 `h-full overflow-hidden` 整頁不捲；設計稿 y 座標以裝置畫面頂端為原點，程式以 `66 - PHONE_STATUS_BAR_H` 這種算式寫出換算而非填換算後的數字。主鍵 104 → 132，說話時縮 72＋淡黃底＋圖示改色＋標籤橫排換「等阿白說完，再按這裡」，240ms 過場。回話卡加收合：說完（speaking → idle）收成單行摘要膠囊（前綴＋前 12 字＋刪節號），新內容一抵達就展開——**同一次變更裡「有新內容」永遠贏過「說完了」**，否則下一輪的第一段會被上一輪的收合蓋掉。新增狀態帶做三重編碼（顏色＋自畫 SVG 圖示＋文字），`useTalk` 補 `error` 第五態。字級放大的例外改用等價做法：網頁沒有 `fontScale` API，改由回話內文自己 `max-height`＋捲動，頁首／角色／主鍵仍固定。舞台的 `role="img"` aria-label 隨狀態帶到位而退場（可見文字優於 aria-label），對應的 `strings.talk.avatar.*` 一併刪除不留死字串。新增 `TalkIcons.tsx`（六個自畫 SVG，不裝圖示套件）。43 檔 691 條全過。✅ **W4「之前聊過的」完成**——新增 `elder/todayLog.ts`（localStorage 版，值與規格對齊 App 那份；App 那條序列化佇列刻意不照抄，`localStorage` 是同步的、讀改寫之間插不了隊，照抄會讓人以為 web 也有那個競態）、`elder/HistoryScreen.tsx`（只留當天、最新在上、自畫 60dp 圓鈕頁首、只有內容層可捲）與 `history` 路由。入口在對講機頁首左側 60dp 圓鈕。寫入點放畫面層（與 App 一致）：`collapsed` 翻成 true 的那一刻就是「答案播完了」，避開 `useTalk` 播放佇列的補播與插嘴競態；同一輪以 ref 去重。`talkSocket` 的 reply 訊框補宣告 `transcript?`（後端 2026-08-07 已送，web 型別缺宣告；web 這支不在「不要動」清單，直接補型別而非像 App 用執行期斷言繞過），`useTalk` 對外露出它——**沒有逐字稿就不寫那一筆**，寧可少一則也不要在「您說」編一句長輩沒講過的話。登出與綁定失效兩處都呼叫 `clearToday()`：逐字只存在本機，手機要交還給家人。依 2026-08-07 選項 1 不渲染「再聽一次」，web 這側連文案鍵都不加（沒有呼叫端的字串就是死字串）。新增 14 條測試，45 檔 709 條全過。✅ **W5a 家屬端三支分頁完成**（W5 拆兩步：W5a 先補畫面、W5b 才換導覽，風險最高的那一批只需處理導覽本身）——新增 `guardian/GuardianTabsProvider.tsx`＋`guardianTabsContext.ts`（「目前這位長輩」＝後端清單第一位的暫用規則集中在一處，含 401 吞掉與 requestId 競態保護）、`TabLanding.tsx` 與 `ReportScreen`／`ProfileScreen`。**刻意做得跟 App 一樣薄**：完整報告與長輩帳密仍在既有詳情頁，這兩頁只負責把人帶進去；要變厚是兩端一起做的設計決定。**不做 `add` 畫面**——App 那支是 6 行的 `Redirect`，防的是 expo-router 的深連結，web 是元件狀態導覽沒有這個入口，真正行為（中央黃鍵）屬 W5b。修掉一個 App 帶過來的不一致：App 的 profile 頁標題寫死「阿公」而 tab 標籤是動態的，web 兩者都讀 `primaryElder`，`profileFallback` 只當讀不到時的退路並在 strings 註明。⚠️ 這三支在 W5b 接上 Tabs 前**沒有使用者入口**，這是拆兩步的已知代價，但已有 7 條測試覆蓋四種狀態。46 檔 716 條全過。✅ **W5b 家屬端五項導覽完成**——`GuardianApp` 由單層 `useScreenStack` 改成兩層：分頁（首頁／報告／通知／我的）切換同一塊內容且導覽列常駐，深層頁（長輩詳情、行程管理）push 在殼之上並讓導覽列消失，認證畫面在殼之外。**兩層共用同一個 stack**（深度 1＝殼、>1＝深層頁），不另開一套狀態——分兩套的話「從詳情返回要回到原本那個分頁」得自己記，而那正是最容易漏的一條。通知由 push 出來的深層頁改為分頁，`GuardianTabsProvider` 掛在 `GuardianApp` 最外層（未登入時它自己不打 API，條件掛載會讓登入那一刻整棵子樹重掛、剛載好的長輩又被丟掉重讀）。中央黃鍵是動作不是分頁（`role` 用 button 不用 tab），按下當刻**重打一次 API 不讀快取**（家屬剛建完第一位長輩時快取可能還是空的），三條分支：有長輩→行程管理頁、沒長輩→回首頁並說明、失敗→顯示原因不靜默失敗，另有 `addBusy` 防連點。新增 `GuardianTabBar` 與 `GuardianTabIcons`（五個自畫 SVG，形狀差異明顯——設計稿雖要求每個圖示都有文字標籤，但形狀相近的圖示在小尺寸下等於沒有）。新增 11 條測試涵蓋分頁切換與 aria-selected、未讀 badge、深層頁隱藏導覽列與返回回到原分頁、未登入無導覽列、中央鍵四種結果。46 檔 727 條全過。✅ **W6 家屬端新畫面完成，辛-30 全批收尾**——新增 `DailySummaryScreen`（日期切換、免責聲明、分享）與 `EditAppointmentScreen`（預填、存檔、刪除），皆用現有 API 不等後端。摘要**照真實型別 `{date, content, created_at}` 做一段純文字**，設計稿的三層結構沒有對應欄位（同 App 那批的核對結論）。改回診**不做**交付稿畫的「誰帶長輩去」與「讓阿白告訴長輩改了」——`ScheduleInput` 沒有 `driver`／`notify_elder`，畫上去只會送出後端收不到的欄位，或更糟：讓家屬以為長輩會被告知而其實不會。分享用 `navigator.share`，桌機沒有時退回複製到剪貼簿而非丟「不支援」；刪除用畫面內確認列不用 `window.confirm`（後者會鎖住整個分頁，雙欄時另一欄按不了）。入口：長輩詳情的摘要區塊加「查看每日摘要」，行程管理頁按「編輯」時**回診走專屬畫面、其餘仍原地編輯**（與 App 同一條規則）。**危急詳情刻意不做**：App 那支是無入口的降級殼（`AppNotification` 連 id 都沒有，從清單導不過去），移植過來一樣是死程式碼；阻塞項在後端，已列辛-29。純函式抽到 `guardianFormat.ts`（Fast Refresh 規則）。新增 17 條測試，47 檔 744 條全過。**架構決議已落地**：`otto-pet-core` 與 `ottoBridge` 已搬至三端共用包 `shared/`，產生器同時寫出兩端產物，web 以 iframe 載入同一份 `renderer.html`——情緒黑名單與 viseme 對嘴只有一份實作（此句原為「待追認」，2026-08-09 完成後更正）。 | 使用者 2026-08-09「要補，因為會用網頁做 demo」、「六批全補，web 與 app 對齊」、「直接掛 Otto renderer」 | L |
| 辛-32 | ✅ **App 端驗收缺失修正**（2026-08-09，使用者指示修驗收報告列出的問題）。**規則 15**：`colors.success`／`colors.danger` 當文字色三處（長輩詳情 2.45:1 與 3.70:1、綁定頁 3.41:1）改用深一階，綁定頁改用共用 `ErrorText`。**規則 6**：`ErrorText` 補圖示做三重編碼（圖示用 danger、文字用 dangerText，正是這兩個 token 的分工）。**規則 1**：`elder/bind`／`login`／`notifications` 三支原生 header 標題補 `headerTitleStyle` 到 22px，不改全域以免家屬端被連帶放大。**規則 4**：行程頁兩組自寫 chip（實測約 43dp）改用共用 `Chip`；順帶修掉共用 Chip 兩種 role 都送 `accessibilityState.selected` 的缺陷（checkbox 應為 `checked`，從交付稿繼承）。**規則 14**：分頁畫面的頁內主鍵改 `outline`——底部常駐中央暖黃鍵，頁內再放暖黃等於每一個分頁都違反。**角色**：`BearAvatar` 與 renderer 降級圖都 `require("akin-hero.png")`（舊角色阿金），前者是無條件顯示；改為直接用同一個 renderer，未就緒時只留光暈不放暫用圖，舊素材刪除。**assertKeysExist**：`theme.ts` 宣告了卻從未實作，改在 `bear:check` 做建置期閘門，同時比對 `theme.ts` 的 `emotionPolicy.blocked` 與 renderer 的 `BLOCKED_EMOTIONS` 是否同一份（兩份分屬 App 與 renderer 兩個世界，型別檢查連不起來，漏改的症狀是「阿白某天開始對長輩生氣」）——已實際移除一個 key 驗證閘門會紅。另清掉 `Section inset` 零採用（綁定碼面板改用它，順帶修好全庫唯一沒對上 radius token 的圓角 12）、兩個 nav 死字串、`elderSeesOnlyNewTime` 未顯示的隱私聲明、`guardian/elder` 空目錄。jest 設定加入 `phosphor-react-native` 轉譯，測試不必再逐支 mock 圖示。21 檔 120 條全過。⏳ 未處理：長輩端「一屏不捲」在 `notifications`（整頁 FlatList）與大字級下的溢位、`emotionPolicy` 的 `maxPerTurn`／`fadeMs` 尚未落地、長輩端用藥狀態的三重編碼（需 StatusPill 接進 notifications）。 | 使用者 2026-08-09「修 App 那邊的驗收問題」 | M |
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| v1.53 | 2026-08-09 | 新增辛-32：修掉 App 端驗收報告列出的 CRITICAL／HIGH——三處色彩對比、ErrorText 三重編碼、長輩端 header 字級、Chip 43dp 與 a11y、規則 14 暖黃衝突、角色頭像仍是舊角色阿金；並把 `assertKeysExist` 從宣告變成 `bear:check` 的建置期閘門（含兩份黑名單一致性）。app 21 檔 120 條全過。 |
| v1.52 | 2026-08-09 | 辛-30 更新：W6 完成，網頁版六批全部到位——每日摘要（切日／分享／免責）與改回診（預填／存檔／刪除確認）皆用現有 API；摘要照真實型別做純文字版，改回診不做契約不存在的兩個控制項；危急詳情刻意不移植（App 那支是無入口的降級殼，阻塞在後端）。web 47 檔 744 條全過。 |
| v1.51 | 2026-08-09 | 辛-30 更新：W5b 完成——網頁版家屬端改五項 Tabs＋中央凸出動作鍵，兩層導覽共用同一個 stack、深層頁隱藏導覽列且返回回到原分頁；通知由深層頁改為分頁；中央鍵按下重打 API 並有三條分支與防連點。web 46 檔 727 條全過。 |
| v1.50 | 2026-08-09 | 辛-30 更新：W5a 完成——網頁版補上家屬端報告／我的兩支落地頁與「目前這位長輩」Provider，刻意與 App 同樣薄；不做 add 畫面（web 無深連結入口）；修掉 App 的 profile 標題寫死不一致。這三支在 W5b 接上 Tabs 前沒有使用者入口。web 46 檔 716 條全過。 |
| v1.49 | 2026-08-09 | 辛-30 更新：W4 完成——網頁版新增當日對話紀錄（localStorage）與「之前聊過的」畫面、對講機頁首入口、登出與綁定失效清除；`talkSocket` 補 `transcript?` 型別宣告，沒有逐字稿就不寫紀錄。web 45 檔 709 條全過。 |
| v1.48 | 2026-08-09 | 辛-30 更新：W3b 完成——對講機改四層絕對定位、一屏不捲、主鍵 132／72 兩態、回話卡收合、狀態帶三重編碼、`useTalk` 補 error 第五態；舞台 aria-label 退場改由狀態帶的可見文字承擔。web 43 檔 691 條全過。 |
| v1.47 | 2026-08-09 | 辛-30 更新：手機外框改為 iPhone 17／17 Pro 實際視埠 402 × 874（邊框改 outline 免得吃掉寬度），為 W3b 的一屏不捲提供可量測的內容區。另修一條全庫平行跑才會紅的等待上限問題（`GuardianApp` 401 守衛），以 `--no-file-parallelism` 實測確認是負載而非產品時序缺陷；刻意只調該條、不調全域，因為 `useTalk.test.ts` 用假時鐘而 `waitFor` 在假時鐘下會推進時間。 |
| v1.46 | 2026-08-09 | 辛-30 更新：W3a 完成——網頁版接上 Otto renderer（iframe 載入與 App 同一份產物），`ottoBridge` 與產生器搬進 `shared/` 讓協定與情緒黑名單只有一份實作；`kinsun-bridge.js` 補瀏覽器回傳路徑。新增 BearStage 8 條 bridge 測試，web 43 檔 683 條、app 21 檔 116 條全過。 |
