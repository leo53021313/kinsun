...
> **版本:** v1.28 | **更新:** 2026-07-30 | **狀態:** 辛-23 追加 main 同步後契約校正（ack 播完維持 thinking、reply 結束才 idle、純文字 reply 直接收尾、error 顯示錯誤態；已補測試）；庚-11（A-10 有字無聲）2026-07-29 由「不改」改判並完成；辛-22（修未註冊工具導致的退化輸出，A＋C 兩層已完成、B 提示詞動態化留待辦）；辛-21（一輪總時間預算 `TURN_BUDGET_SECONDS=30`，已完成）；辛-20（修對講機改走 WS 後定位失效，已完成）。第 5 層 RAG 正式化、版本發布、單庫原地遷移與個人 Supabase 驗收均已完成；甲～庚批、內測基礎建設（D-73）與辛批其餘狀態維持原表。
...
| v1.26 | 2026-07-30 | 將 Prototype 視覺工項改編為辛-23，與 main 已存在的辛-18 附近地點搜尋區分；同步整合 WebSocket 對講、提醒鈴鐺與 `POST /turns` 降級契約。 |
| v1.27 | 2026-07-30 | 辛-23 追加 main 同步後 WS 狀態收尾修正與回歸測試：ack 播完維持 thinking、reply 最後一段才 idle、error 不再顯示成待機，並阻止較舊播放回呼覆蓋新狀態。 |
| v1.28 | 2026-07-30 | 辛-23 補 TTS 純文字降級：WS reply 的 `audio_url=""` 時直接離開 thinking；有語音仍由播放完成事件收尾，新增 3 條回歸測試。 |
