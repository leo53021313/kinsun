...
def test_system_prompt_keeps_the_ai_honesty_rule():
    """⚠️ 這條界線不可刪：長輩陪伴助理不得假裝成真的孫子。

    與下一個測試成對——下一個測試限縮它的**時機**，不是取消它。任何一次改動讓
    這條規則消失，都要先想清楚是不是把承重的東西當成了「像是沒做完」。
    """
    assert "你是 AI，不要假裝是真人或家人" in SYSTEM_PROMPT


def test_system_prompt_forbids_the_ai_disclaimer_at_the_crisis_moment():
    """⚠️ 回歸防線，非行為驗證——這一條沒有程式層防線，只有這句話。

    2026-08-01 正式環境實況：長輩連說兩句「想去西方極樂世界」（都正確判 L2、
    通報了家屬），第三句問「為什麼一定要找家人 而不是要找你」，金孫回：
    「阿公，我是AI呀，沒辦法給您真正的陪伴。家人隨時都在，快去找他們說說話好嗎？」

    那一刻說這句話是把一個正在表達自傷意念的人推開；而同一輪注入的長期記憶裡
    明寫著「長者將AI助手視為家人，並稱呼其為『金孫』」——自己打自己臉。
    四句回覆有三句是「快去找家人」，沒有一句留下來陪。

    根因**不是**「你是 AI」那條規則有錯（它承重，見上一個測試），是它在最糟的
    時機開火。修法是限縮時機，不是刪規則。
    """
    assert "我只是 AI" in SYSTEM_PROMPT
    assert "把他推開" in SYSTEM_PROMPT
    assert "不要每一句都叫他去找家人" in SYSTEM_PROMPT


def test_system_prompt_excludes_stating_the_present_from_reminder_offers():
    """⚠️ 回歸防線，非行為驗證——這一條沒有程式層防線，只有這句話。

    2026-08-01 正式環境實況：長輩說「重訓的時間到了」（在陳述現在，不是交代未來），
    金孫沒有反問就直接呼叫 `create_schedule`，憑空建了一筆「8月1日 14:00 重訓」——
    當時是 13:59，一分鐘後真的跳提醒。長輩從沒要求記任何東西。

    ⚠️ 為什麼只有提示詞、沒有程式防線（三種都評估過，都不成立）：
    * 「時刻必須出自長輩原話」——會打死正規流程。提示詞本來就要金孫提議時刻
      （「那我八點四十五先叫您好嗎」），長輩只回一句「好」，原話裡沒有任何時刻。
    * 「排定時刻不得早於現在」——本案排在一分鐘後，擋不到。
    * 「本輪回覆沒有徵詢句就撤銷」——排程已經寫進庫，撤銷是第二次寫入，比誤建更糟。
    真正的界線只有 `_requires_utterance`（長輩這一輪有沒有開口），它擋不了「開了口
    但不是在交代事情」。這是**已知的殘餘風險**，不要以為這個洞補完了。
    """
    assert "時間到了" in SYSTEM_PROMPT
    assert "不是要你記" in SYSTEM_PROMPT
